...
Module audit epoch `hdp-module-audit-2026-08-28-actionability-v2`, SHA-256
`af71fe2eb25e672e6bd13898b770123b2d1321849b6a4b00b6ec9486fa3734b2`.
...
- Formalized objects: **0**.
- Remaining objects: **119**.
- Denominator: **119**. Percentage: **0.00%**.
- Reported separately and excluded from the denominator: **11** skipped, **0** deferred.
- Status counts: `READY`=119, `SKIPPED`=11.
- No row is `UNCLASSIFIED`. Weakened rows: 0. Discrepancy rows: 0.

**Why the percentage is 0.00% while 76 rows carry compiling Lean.** A row becomes a
...
the printed row. Chapter 2 has no completed per-row audit yet, so every row with
Lean content is still `READY`, and the gate reports that honestly rather than
counting compiling declarations as coverage.
...
| `HDP-02-EX-2.8.5` | Exercise 2.8.5 | exercise | `READY` | `NumStability.HDP.Contract.hdp_02_hex_h2_d8_d5` |
