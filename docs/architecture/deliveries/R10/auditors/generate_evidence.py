#!/usr/bin/env python3
"""Regenerate the byte-derived R10 delivery ledgers.
...
structure; only the identity pins and closed totals are R10-specific.
"""
...
DELIVERY = Path("docs/architecture/deliveries/R10")
TEST_PREFIX = "NumStabilityTest/Reorganization/R10/"
COPIES = {
    "DECLARATION_ROUTES.tsv": "B0012-declaration-routes.tsv",
    "PRIVATE_CLOSURE.tsv": "B0012-private-closure.tsv",
    "PRIVATE_NORMALIZATION.tsv": "B0012-private-normalization.tsv",
    "REALIZED_IMPORTS.tsv": "B0012-post-move-import-manifest.tsv",
    "TEST_MATRIX.tsv": "B0012-test-plan.tsv",
}
EXPECTED_EVIDENCE = {
    "CHANGED_PATHS.md",
...
EXPECTED_TOTALS = {
    "baseline_declarations": 225,
    "public_declarations": 222,
    "private_declarations": 3,
    "relocated_declarations": 225,
    "retained_declarations": 0,
}
EXPECTED_ROWS = 18
EXPECTED_CHANGES = Counter({"modified": 9, "byte_identical": 9})
...
def verify_control(control: Path) -> None:
    record = control / BRANCH / "B0012.json"
    if not record.is_file():
        raise RuntimeError("activated B0012.json is missing from the control checkout")
    payload = json.loads(record.read_text(encoding="utf-8"))
    if payload.get("base_sha") != BASE_SHA:
        raise RuntimeError("B0012 is not based on the exact C0006 code commit")
    if payload.get("status") not in {"active", "accepted"}:
        raise RuntimeError(f"B0012 status is {payload.get('status')!r}")
...
def retention_bytes(repo: Path, control: Path) -> bytes:
    inventory = read_tsv(control / BRANCH / "B0012-inventory.tsv")
    module_routes = {row["owner_module"]: row
                     for row in read_tsv(control / BRANCH / "B0012-module-routes.tsv")}
    routes = read_tsv(control / BRANCH / "B0012-declaration-routes.tsv")
...
def main() -> int:
    ...
    if problems:
        print("R10 evidence generation check failed:", file=sys.stderr)
