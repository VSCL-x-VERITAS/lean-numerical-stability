#!/usr/bin/env python3
"""Deterministic R10 postimage verifier.
...
command byte-for-byte.  The R10 planning control has no source-command ledger,
...
delivered inputs.  This program therefore verifies rather than re-renders:
  ...
  * every compatibility wrapper is re-derived from B0012-wrapper-imports.tsv
    and required to be exactly that import sequence and nothing else;
  * every destination import sequence is re-derived from
    B0012-post-move-import-manifest.tsv;
  * every test module is re-derived from B0012-test-plan.tsv;
...
def main() -> int:
    ...
    wrappers = read_tsv(control / BRANCH / "B0012-wrapper-imports.tsv")
    for row in wrappers:
    ...
    manifest_rows = read_tsv(control / BRANCH / "B0012-post-move-import-manifest.tsv")
    by_module: dict[str, list[dict[str, str]]] = {}
    for row in manifest_rows:
        by_module.setdefault(row["module"], []).append(row)
    for row in read_tsv(control / BRANCH / "B0012-destinations.tsv"):
        module = row["module"]
    ...
    for row in read_tsv(control / BRANCH / "B0012-test-plan.tsv"):
    ...
    if problems:
        print("R10 materialization verify FAILED", file=sys.stderr)
