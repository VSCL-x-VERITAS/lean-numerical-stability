...
lemma IsPreBrownian.hasIndepIncrements [h : IsPreBrownian X P] : HasIndepIncrements X P := by
  ...
  · simp_rw [IsPreBrownian.covariance_fun_eval]
    ...
    · simp_rw [← this j i hij.symm (by omega), NNReal.coe_min, min_comm]
      ring
    ...
    rw [min_eq_left (NNReal.coe_le_coe.mpr (ht h1)),
      min_eq_left (NNReal.coe_le_coe.mpr (ht h')),
      min_eq_left (NNReal.coe_le_coe.mpr (ht h2)),
      min_eq_left (NNReal.coe_le_coe.mpr (ht h3))]
    simp
    ...
    · rw [← ContinuousLinearMap.toBilinForm_inj]
      ...
      · congrm ∑ i, ∑ j, _ * ?_
        ...
        · by_cases hij : i.1 ≤ j.1
          · simpa [NNReal.coe_min, min_eq_left (NNReal.coe_le_coe.mpr hij)] using h3 i j hij
          · have hji : j.1 ≤ i.1 := le_of_not_ge hij
            rw [covariance_comm]
            simpa [NNReal.coe_min, min_eq_right (NNReal.coe_le_coe.mpr hji)] using h3 j i hji
...
lemma IsPreBrownian.shift [h : IsPreBrownian X P] (t₀ : ℝ≥0) :
  ...
  · have := h.isGaussianProcess.isProbabilityMeasure
    rw [covariance_fun_sub_left, covariance_fun_sub_right, covariance_fun_sub_right,
      h.covariance_eval, h.covariance_eval, h.covariance_eval, h.covariance_eval]
    simp only [NNReal.coe_min]
    simp [hst]
    any_goals exact (h.isGaussianProcess.hasGaussianLaw_eval _).memLp_two
    exact h.isGaussianProcess.hasGaussianLaw_sub.memLp_two
...
lemma IsPreBrownian.indepFun_shift [h : IsPreBrownian X P] (hX : ∀ t, Measurable (X t)) (t₀ : ℝ≥0) :
  ...
  · rintro s ⟨t, ht : t ≤ t₀⟩
    have := h.isGaussianProcess.isProbabilityMeasure
    rw [covariance_fun_sub_left, h.covariance_eval, h.covariance_eval]
    simp only [NNReal.coe_min]
    rw [min_eq_right, min_eq_right, sub_self]
    · exact NNReal.coe_le_coe.mpr ht
    · exact NNReal.coe_le_coe.mpr <| ht.trans <| le_add_right (le_refl t₀)
...
lemma IsPreBrownian.inv [h : IsPreBrownian X P] :
  ...
  · have := h.isGaussianProcess.isProbabilityMeasure
    ...
    rw [NNReal.coe_min, min_eq_right]
    · norm_cast
      field_simp
    have hsR : (0 : ℝ) < (s : ℝ) := NNReal.coe_pos.mpr (pos_of_ne_zero hs)
    have hstR : (s : ℝ) ≤ (t : ℝ) := NNReal.coe_le_coe.mpr hst
    exact one_div_le_one_div_of_le hsR hstR
...
lemma ContinuousMap.borel_eq_iSup_comap_eval [SecondCountableTopology X] [SecondCountableTopology Y]
  ...
  · refine iSup_le fun x ↦ ?_
    have hx : Measurable (fun b : C(X, Y) ↦ b x) := Continuous.measurable (by fun_prop)
    simpa only [BorelSpace.measurable_eq] using hx.comap_le
...
lemma ContinuousMap.measurable_iff_eval {α : Type*} [MeasurableSpace α]
  ...
  constructor
  · intro hg x
    have hx : Measurable (fun f : C(X, Y) ↦ f x) := Continuous.measurable (by fun_prop)
    exact hx.comp hg
  · intro h
    apply Measurable.of_comap_le
    calc
      MeasurableSpace.comap g (inferInstance : MeasurableSpace C(X, Y)) =
          MeasurableSpace.comap g
            (⨆ x : X, (inferInstance : MeasurableSpace Y).comap fun f ↦ f x) :=
        congrArg (MeasurableSpace.comap g)
          ContinuousMap.measurableSpace_eq_iSup_comap_eval
      _ = ⨆ x : X, MeasurableSpace.comap g
          ((inferInstance : MeasurableSpace Y).comap fun f ↦ f x) :=
        MeasurableSpace.comap_iSup
      _ ≤ _ := iSup_le fun x ↦ by
        simpa only [MeasurableSpace.comap_comp, Function.comp_def] using (h x).comap_le
