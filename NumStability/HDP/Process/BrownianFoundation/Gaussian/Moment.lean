...
-- u sub with y = x^2 / (2σ²) or x = σ √(2y).
  ...
  _ = σ ^ (2 * n) * 2 ^ n / √Real.pi * Real.Gamma ((n : ℝ) + (1 : ℝ) / 2) := by
    ...
    have hexp : ((2 * (n : ℝ) + 1 - 2) / 2) = -(1 : ℝ) / 2 + n := by ring
    rw [Real.sq_sqrt zero_le_two, mul_pow, hexp, hpow]
    ring
  -- 8. ... = σ^(2n) (2n - 1)!!
