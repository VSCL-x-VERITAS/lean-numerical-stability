...
open scoped ENNReal NNReal Topology RealInnerProductSpace InnerProductSpace Matrix MatrixOrder
...
lemma charFun_stdGaussian (t : E) : charFun (stdGaussian E) t = Complex.exp (- ‖t‖ ^ 2 / 2) := by
  ...
  · simp_rw [sum_inner, Complex.ofReal_sum, Finset.sum_mul, Complex.exp_sum]
    calc
      _ = ∏ i, ∫ x : ℝ, Complex.exp
          (⟪x • stdOrthonormalBasis ℝ E i, t⟫ * Complex.I) ∂gaussianReal 0 1 := by
        convert integral_fintype_prod_eq_prod
          (μ := fun _ : Fin (Module.finrank ℝ E) ↦ gaussianReal 0 1)
          (fun i x ↦ Complex.exp (⟪x • stdOrthonormalBasis ℝ E i, t⟫ * Complex.I)) using 1
      _ = _ := by
        simp_rw [real_inner_smul_left, mul_comm _ (⟪_, _⟫), Complex.ofReal_mul,
          ← charFun_apply_real, charFun_gaussianReal]
        simp only [Complex.ofReal_zero, mul_zero, zero_mul, NNReal.coe_one, Complex.ofReal_one,
          one_mul, zero_sub]
        simp_rw [← Complex.exp_sum, Finset.sum_neg_distrib, ← Finset.sum_div,
          ← Complex.ofReal_pow, ← Complex.ofReal_sum,
          ← (stdOrthonormalBasis ℝ E).norm_sq_eq_sum_sq_inner_right, neg_div]
  · exact Measurable.aemeasurable (by fun_prop)
...
lemma charFunDual_stdGaussian (L : StrongDual ℝ E) :
  ...
  have h_int : (∫ x, (L x : ℂ) ∂stdGaussian E) =
      Complex.ofReal (∫ x, L x ∂stdGaussian E) := integral_complex_ofReal
  rw [IsGaussian.charFunDual_eq, variance_dual_stdGaussian, h_int, isCentered_stdGaussian]
  simp [neg_div]
...
/-- A deterministic covariance factor.  For a positive semidefinite matrix `S`, this is a
matrix `A` such that `A * star A = S`; outside that case it is zero. -/
noncomputable def covarianceFactor (S : Matrix ι ι ℝ) : Matrix ι ι ℝ :=
  by
    classical
    exact if hS : S.PosSemidef then
      star (Classical.choose (Matrix.posSemidef_iff_eq_conjTranspose_mul_self.mp hS))
    else 0

lemma covarianceFactor_mul_star (S : Matrix ι ι ℝ) (hS : S.PosSemidef) :
    covarianceFactor S * star (covarianceFactor S) = S := by
  rw [covarianceFactor, dif_pos hS]
  let B := Classical.choose (Matrix.posSemidef_iff_eq_conjTranspose_mul_self.mp hS)
  have hB : S = Bᴴ * B :=
    Classical.choose_spec (Matrix.posSemidef_iff_eq_conjTranspose_mul_self.mp hS)
  change Bᴴ * star Bᴴ = S
...
def multivariateGaussian (μ : EuclideanSpace ℝ ι) (S : Matrix ι ι ℝ) :
  ...
  (stdGaussian (EuclideanSpace ℝ ι)).map
    (fun x ↦ μ + toEuclideanCLM (𝕜 := ℝ) (covarianceFactor S) x)
...
instance isGaussian_multivariateGaussian : IsGaussian (multivariateGaussian μ S) := by
  have h : (fun x ↦ μ + x) ∘ ((toEuclideanCLM (𝕜 := ℝ) (covarianceFactor S))) =
    (fun x ↦ μ + (toEuclideanCLM (𝕜 := ℝ) (covarianceFactor S)) x) := rfl
...
lemma covarianceBilin_multivariateGaussian (hS : S.PosSemidef) :
  ...
  have h_adjoint :
      (toEuclideanCLM (𝕜 := ℝ) (covarianceFactor S)).adjoint =
        toEuclideanCLM (𝕜 := ℝ) (star (covarianceFactor S)) := by
    calc
      _ = star (toEuclideanCLM (𝕜 := ℝ) (covarianceFactor S)) :=
        (ContinuousLinearMap.star_eq_adjoint _).symm
      _ = _ := ((toEuclideanCLM (n := ι) (𝕜 := ℝ)).map_star' (covarianceFactor S)).symm
  rw [h_adjoint]
  calc inner ℝ x ((toEuclideanCLM (𝕜 := ℝ) (covarianceFactor S))
      (toEuclideanCLM (𝕜 := ℝ) (star (covarianceFactor S)) y))
  _ = inner ℝ x (toEuclideanCLM (𝕜 := ℝ) S y) := by
    congr 1
    have : (toEuclideanCLM (𝕜 := ℝ) (covarianceFactor S)).comp
        (toEuclideanCLM (𝕜 := ℝ) (star (covarianceFactor S))) =
          toEuclideanCLM (𝕜 := ℝ) (covarianceFactor S * star (covarianceFactor S)) := by
      rw [map_mul]
      rfl
    rw [covarianceFactor_mul_star S hS, ContinuousLinearMap.ext_iff] at this
    rw [← this y]
    simp
...
lemma covariance_eval_multivariateGaussian (hS : S.PosSemidef) (i j : ι) :
  ...
  have (i : ι) : (fun x : EuclideanSpace ℝ ι ↦ x i) =
      fun x ↦ ⟪EuclideanSpace.basisFun ι ℝ i, x⟫ := by
    ext
...
noncomputable def _root_.EuclideanSpace.restrict₂ {ι 𝕜 : Type*} [RCLike 𝕜]
    {I J : Finset ι} (hIJ : I ⊆ J) :
...
lemma measurePreserving_restrict_multivariateGaussian (hS : S.PosSemidef) (hJI : J ⊆ I) :
  ...
  map_eq := by
    ...
    · have (i : J) : (fun u ↦ ⟪(EuclideanSpace.basisFun J ℝ).toBasis i, u⟫) ∘
          EuclideanSpace.restrict₂ hJI = fun u ↦ u ⟨i.1, hJI i.2⟩ := by
        ext
        simp [PiLp.inner_apply, real_inner_eq_mul]
...
lemma multivariateGaussian_zero_one [Fintype ι] :
  ...
  have hOne : (1 : Matrix ι ι ℝ).PosSemidef := by
    rw [← diagonal_one]
    exact Matrix.PosSemidef.diagonal fun _ ↦ zero_le_one
  apply IsGaussian.ext
  · simp
  · rw [covarianceBilin_multivariateGaussian hOne, covarianceBilin_stdGaussian]
    ext x y
    simp [ContinuousBilinForm.ofMatrix_apply, EuclideanSpace.real_inner_eq, dotProduct,
