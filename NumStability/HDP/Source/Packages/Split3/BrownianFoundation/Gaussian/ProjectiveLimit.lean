...
lemma covariance_eval_gaussianProjectiveFamily (I : Finset ℝ≥0) (s t : I) :
  ...
  have (u : I) : (fun x : EuclideanSpace ℝ I ↦ x u) =
      fun x ↦ ⟪EuclideanSpace.basisFun I ℝ u, x⟫ := by
    funext x
    exact (EuclideanSpace.basisFun_inner I ℝ x u).symm
...
lemma variance_eval_gaussianProjectiveFamily {I : Finset ℝ≥0} (s : I) :
  ...
  rw [← covariance_self, covariance_eval_gaussianProjectiveFamily]
  norm_num
  ...
  map_eq := by
    ...
    · conv =>
      ...
      · norm_cast
        ...
        rw [two_mul]
        exact add_le_add
          (min_le_left (((s : I) : ℝ≥0) : ℝ) (((t : I) : ℝ≥0) : ℝ))
          (min_le_right (((s : I) : ℝ≥0) : ℝ) (((t : I) : ℝ≥0) : ℝ))
      · exact IsGaussian.integrable_id
