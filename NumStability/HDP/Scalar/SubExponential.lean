...
/-! The source-facing `ψ₁` gauge.  The positive finite-scale and
exponential-integrability conditions are explicit at the contract boundary. -/
  ...
  · rintro ⟨hX, hfinite⟩
    let A : Set ℝ≥0∞ := {t |
      t ≠ 0 ∧ t ≠ ∞ ∧
        Integrable (fun ω => Real.exp (|X ω| / t.toReal)) μ ∧
        (∫ ω, Real.exp (|X ω| / t.toReal) ∂μ) ≤ 2}
    have hnotall : ¬ ∀ t ∈ A, t = ∞ := by
      intro hall
      have : sInf A = ∞ := sInf_eq_top.mpr hall
      exact (ne_of_lt hfinite) this
    push_neg at hnotall
    rcases hnotall with ⟨t, ht, htnot⟩
    have ht0 : t ≠ 0 := ht.1
    have htTop : t ≠ ∞ := ht.2.1
    have htpos : 0 < t.toReal := ENNReal.toReal_pos ht0 htTop
    refine ⟨t.toReal, ?_⟩
    ...
    · exact ht.2.2.2
  · rintro ⟨K, hK⟩
    rcases hK with ⟨hX, hKpos, hInt, hbound⟩
    refine ⟨hX, ?_⟩
    let t : ℝ≥0∞ := ENNReal.ofReal K
    have ht : t ∈ {s : ℝ≥0∞ |
        s ≠ 0 ∧ s ≠ ∞ ∧
