...
/-! The exact source-facing proposition for Exercise 2.6.9 is kept local to
the semantic module. It is not an outbound split contract, so importing a
foreign signature here would unnecessarily widen the provider surface. -/
