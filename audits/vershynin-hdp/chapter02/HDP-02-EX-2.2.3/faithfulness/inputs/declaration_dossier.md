# Declaration dossier for HDP-02-EX-2.2.3
...
```lean
∀ (x : Real), Real.instLE.le (Real.cosh x) (Real.exp (instHDiv.hDiv (instHPow.hPow x 2) 2))
```
...
```lean
∀ (x : Real),
  @LE.le.{0} Real Real.instLE (Real.cosh x)
    (Real.exp
...
### D007: `Nat.instAtLeastTwoHAddOfNat`
