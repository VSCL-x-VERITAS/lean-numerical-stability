...
∀ {ι : Type u_1} {Ω : Type u_2} [inst : Fintype.{u_1} ι] [inst_1 : MeasurableSpace.{u_2} Ω]
  ...
  {K : ι → Real}
  (hX : ∀ (i : ι), @LocalDef002.{u_2} Ω inst_1 μ (X i) (K i))
  (hIndep :
    ...
    And (@LE.le.{0} Real Real.instLE (@OfNat.ofNat.{0} Real (nat_lit 1) (@One.toOfNat1.{0} Real Real.instOne)) C)
      (And
        ...
        (@LE.le.{0} ENNReal (@Preorder.toLE.{0} ENNReal (@PartialOrder.toPreorder.{0} ENNReal ENNReal.instPartialOrder))
          (@LocalDef001.{u_2} Ω inst_1 μ fun (ω : Ω) =>
            @Finset.sum.{u_1, 0} ι Real Real.instAddCommMonoid (@Finset.univ.{u_1} ι inst) fun (i : ι) => X i ω)
          (ENNReal.ofReal
...
{Ω : Type u_1} →
  [inst : MeasurableSpace Ω] →
    MeasureTheory.Measure Ω → (Ω → Real) → LocalDef008 → Real → Prop
```
...
fun {Ω} [MeasurableSpace Ω] μ X x =>
  LocalDef007 (fun x => Real → Prop) x
    (fun _ => LocalDef011 μ X)
    (fun _ => LocalDef006 μ X)
    (fun _ => LocalDef010 μ X)
    (fun _ => LocalDef009 μ X) fun _ =>
    LocalDef002 μ X
```
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X K =>
  And (Measurable X) (And (Real.instLT.lt 0 K) (LocalDef013 μ X K))
```
...
```lean
(motive : LocalDef008 → Sort u_1) →
  (x : LocalDef008) →
    (Unit → motive LocalDef018) →
      (Unit → motive LocalDef015) →
        (Unit → motive LocalDef017) →
          (Unit → motive LocalDef016) →
            (Unit → motive LocalDef004) → motive x
```
...
fun motive x h_1 h_2 h_3 h_4 h_5 =>
  LocalDef014 x (h_1 Unit.unit) (h_2 Unit.unit) (h_3 Unit.unit)
...
```lean
{motive : LocalDef008 → Sort u} →
  (t : LocalDef008) →
    motive LocalDef018 →
      motive LocalDef015 →
        motive LocalDef017 →
          motive LocalDef016 →
            motive LocalDef004 → motive t
```
...
fun {motive} t tail moment squareWindow squarePoint linearMGF =>
  LocalDef019 tail moment squareWindow squarePoint linearMGF t
```
