# Declaration dossier for HDP-02-EQ-2.11
...
theorem standardNormalLpNormGrowth :
    ∃ C : ℝ, 0 < C ∧
      ∀ p : ℝ, 1 ≤ p →
        (∫ x : ℝ, |x| ^ p ∂(gaussianReal 0 1)) ^ (1 / p) ≤ C * Real.sqrt p
```
...
Exists fun C =>
  And (Real.instLT.lt 0 C)
    (∀ (p : Real),
      Real.instLE.le 1 p →
        Real.instLE.le
          (instHPow.hPow (MeasureTheory.integral (ProbabilityTheory.gaussianReal 0 1) fun x => instHPow.hPow (abs x) p)
            (instHDiv.hDiv 1 p))
          (instHMul.hMul C p.sqrt))
```
        ...
        @LE.le.{0} Real Real.instLE
          (@HPow.hPow.{0, 0, 0} Real Real Real (@instHPow.{0, 0} Real Real Real.instPow)
            (@MeasureTheory.integral.{0, 0} Real Real Real.normedAddCommGroup
              ...
              fun (x : Real) =>
              @HPow.hPow.{0, 0, 0} Real Real Real (@instHPow.{0, 0} Real Real Real.instPow)
                (@abs.{0} Real Real.lattice Real.instAddGroup x) p)
            (@HDiv.hDiv.{0, 0, 0} Real Real Real
              (@instHDiv.{0} Real (@DivInvMonoid.toDiv.{0} Real Real.instDivInvMonoid))
              (@OfNat.ofNat.{0} Real (nat_lit 1) (@One.toOfNat1.{0} Real Real.instOne)) p))
          (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul) C (Real.sqrt p)))
```
