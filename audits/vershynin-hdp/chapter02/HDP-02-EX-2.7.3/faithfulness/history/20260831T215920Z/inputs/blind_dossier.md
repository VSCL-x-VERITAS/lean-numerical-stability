      ...
      @LT.lt.{0} Real Real.instLT (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero)) alpha →
        ∀ (i j : LocalDef002) {Ki : Real},
          @LT.lt.{0} Real Real.instLT (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero)) Ki →
            @LocalDef001.{u_1} Omega inst mu X alpha i Ki →
              @Exists.{1} Real fun (Kj : Real) =>
                And
                  ...
                  (And
                    ...
                    (@LocalDef001.{u_1} Omega inst mu X alpha j Kj)))
```
...
{Ω : Type u_1} →
  [inst : MeasurableSpace Ω] →
    MeasureTheory.Measure Ω →
      (Ω → Real) → Real → LocalDef002 → Real → Prop
```
...
fun {Ω} [MeasurableSpace Ω] μ X α x x_1 =>
  LocalDef005 (fun x x_2 => Prop) x x_1 fun i K =>
    And (Real.instLT.lt 0 α)
      (LocalDef003 μ (fun ω => instHPow.hPow (abs (X ω)) α) i K)
```
...
fun {Ω} [MeasurableSpace Ω] μ X x =>
  LocalDef009 (fun x => Real → Prop) x
    (fun _ => LocalDef014 μ X)
    (fun _ => LocalDef007 μ X)
    (fun _ => LocalDef006 μ X) fun _ =>
    LocalDef008 μ X
```
...
```lean
(motive : LocalDef002 → Real → Sort u_1) →
  (x : LocalDef002) →
    (x_1 : Real) →
      ((i : LocalDef002) → (K : Real) → motive i K) → motive x x_1
```
...
fun motive x h_1 h_2 h_3 h_4 =>
  LocalDef016 x (h_1 Unit.unit) (h_2 Unit.unit)
...
```lean
{motive : LocalDef004 → Sort u} →
  (t : LocalDef004) →
    motive LocalDef013 →
      motive LocalDef011 →
        motive LocalDef010 →
          motive LocalDef012 → motive t
```
...
fun {motive} t tail moment absoluteMGF onePoint =>
  LocalDef018 tail moment absoluteMGF onePoint t
```
