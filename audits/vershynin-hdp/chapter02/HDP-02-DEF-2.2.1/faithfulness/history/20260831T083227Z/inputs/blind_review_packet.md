...
```lean
And (Eq (PMF.instFunLike.coe LocalDef001.law (-1)) (1 / 2))
  (And (Eq (PMF.instFunLike.coe LocalDef001.law 1) (1 / 2))
    (∀ {p : NNReal} (hp : instPartialOrderNNReal.le p 1),
      Iff
        (Eq (PMF.map LocalDef003 (PMF.bernoulli p hp))
          LocalDef001.law)
        (Eq p (1 / 2))))
```
...
And
  ...
  (And
    ...
    (∀ {p : NNReal}
      ...
      Iff
        (@Eq.{1} (PMF.{0} Real)
          (@PMF.map.{0, 0} Bool Real LocalDef003
            (PMF.bernoulli p hp))
          (LocalDef002
            LocalDef001))
        (@Eq.{1} NNReal p
...
```lean
{ law := LocalDef008,
  mass_one := LocalDef011,
  mass_neg_one := LocalDef010,
  affine_bernoulli_iff := ⋯, mean := LocalDef012,
  variance := LocalDef013,
  abs_mean := LocalDef009 }
```
...
(law : PMF Real) →
  Eq (PMF.instFunLike.coe law 1) (1 / 2) →
    Eq (PMF.instFunLike.coe law (-1)) (1 / 2) →
      (∀ {p : NNReal} (hp : instPartialOrderNNReal.le p 1),
          Iff (Eq (PMF.map LocalDef003 (PMF.bernoulli p hp)) law)
            (Eq p (1 / 2))) →
...
```lean
Eq (MeasureTheory.integral LocalDef008.toMeasure fun x => abs x) 1
```
...
```lean
Eq (PMF.instFunLike.coe LocalDef008 (-1)) (1 / 2)
```
...
```lean
Eq (MeasureTheory.integral LocalDef008.toMeasure fun x => x) 0
```
...
```lean
Eq
  (MeasureTheory.integral LocalDef008.toMeasure fun x =>
