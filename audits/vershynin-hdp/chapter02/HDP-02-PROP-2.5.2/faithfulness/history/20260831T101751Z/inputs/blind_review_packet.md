              ...
              @Ne.{1} LocalDef002 j
                ...
                ∀ {Ki : Real},
                  @LT.lt.{0} Real Real.instLT
                    ...
                    @LocalDef001.{u_1} Ω inst μ X i Ki →
                      @Exists.{1} Real fun (Kj : Real) =>
                        And
                          ...
                          (And
                            ...
                            (@LocalDef001.{u_1} Ω inst μ X j Kj)))
          ...
          And
            ...
            ∀ (i j : LocalDef002) {Ki : Real},
              @LT.lt.{0} Real Real.instLT (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero))
                  Ki →
                @LocalDef001.{u_2} Ω inst μ X i Ki →
                  @Exists.{1} Real fun (Kj : Real) =>
                    And
                      (@LT.lt.{0} Real Real.instLT
                        (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero)) Kj)
                      (And
                        (@LE.le.{0} Real Real.instLE Kj
                          (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul) C Ki))
                        (@LocalDef001.{u_2} Ω inst μ X j Kj))))
```
...
{Ω : Type u_1} →
  [inst : MeasurableSpace Ω] →
    MeasureTheory.Measure Ω → (Ω → Real) → LocalDef002 → Real → Prop
```
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X K =>
  And (Measurable X) (And (Real.instLT.lt 0 K) (LocalDef015 μ X K))
```
...
fun motive x h_1 h_2 h_3 h_4 h_5 =>
  LocalDef016 x (h_1 Unit.unit) (h_2 Unit.unit) (h_3 Unit.unit)
...
```lean
{motive : LocalDef002 → Sort u} →
  (t : LocalDef002) →
    motive LocalDef010 →
      motive LocalDef007 →
        motive LocalDef009 →
          motive LocalDef008 →
            motive LocalDef003 → motive t
```
...
fun {motive} t tail moment squareWindow squarePoint linearMGF =>
  LocalDef017 tail moment squareWindow squarePoint linearMGF t
```
