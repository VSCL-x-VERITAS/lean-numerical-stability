...
∀ {Omega : Type u_1} [inst : MeasurableSpace Omega] {mu : MeasureTheory.Measure Omega}
  ...
  (Exists fun i =>
      And (Ne i LocalDef005)
        (Exists fun K => And (Real.instLT.lt 0 K) (LocalDef003 mu X i K))) →
    Exists fun K =>
  ...
  (@Exists.{1} LocalDef004
    ...
    @Exists.{1} Real fun (K : Real) =>
      And (@LT.lt.{0} Real Real.instLT (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero)) K)
        (@LocalDef001.{u_1} Omega inst mu X
          LocalDef002 K)
```
...
{Ω : Type u_1} →
  [inst : MeasurableSpace Ω] →
    MeasureTheory.Measure Ω → (Ω → Real) → LocalDef004 → Real → Prop
```
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X x =>
  LocalDef014 (fun x => Real → Prop) x
    (fun _ => LocalDef021 μ X)
    (fun _ => LocalDef013 μ X)
    (fun _ => LocalDef020 μ X)
    (fun _ => LocalDef019 μ X) fun _ =>
    LocalDef012 μ X
```
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X K =>
  And (Measurable X) (And (Real.instLT.lt 0 K) (LocalDef022 μ X K))
```
...
```lean
(motive : LocalDef010 → Sort u_1) →
  (x : LocalDef010) →
    (Unit → motive LocalDef026) →
      (Unit → motive LocalDef002) →
        (Unit → motive LocalDef023) →
          (Unit → motive LocalDef025) → motive x
```
...
fun motive x h_1 h_2 h_3 h_4 =>
  LocalDef024 x (h_1 Unit.unit) (h_2 Unit.unit)
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X K =>
  And (Measurable X) (And (Real.instLT.lt 0 K) (LocalDef029 μ X K))
```
      ...
      (Unit → motive LocalDef015) →
        (Unit → motive LocalDef017) →
          (Unit → motive LocalDef016) →
            (Unit → motive LocalDef005) → motive x
```
...
fun motive x h_1 h_2 h_3 h_4 h_5 =>
  LocalDef030 x (h_1 Unit.unit) (h_2 Unit.unit) (h_3 Unit.unit)
...
```lean
{motive : LocalDef010 → Sort u} →
  (t : LocalDef010) →
    motive LocalDef026 →
      motive LocalDef002 →
        motive LocalDef023 →
          motive LocalDef025 → motive t
```
...
fun {motive} t tail moment absoluteMGF onePoint =>
  LocalDef031 tail moment absoluteMGF onePoint t
```
...
fun {motive} t tail moment squareWindow squarePoint linearMGF =>
  LocalDef032 tail moment squareWindow squarePoint linearMGF t
```
