# Declaration dossier for HDP-02-EQ-2.5
  ...
  Measurable S →
    ∀ {lam t : Real},
      Real.instLT.lt 0 lam →
        MeasureTheory.Integrable (fun ω => Real.exp (instHMul.hMul lam (S ω))) μ →
          And
            (Eq (μ.real (Set.preimage S (Set.Ici t)))
              (μ.real
                (Set.preimage (fun ω => Real.exp (instHMul.hMul lam (S ω)))
                  (Set.Ici (Real.exp (instHMul.hMul lam t))))))
            (Real.instLE.le (μ.real (Set.preimage S (Set.Ici t)))
              (instHMul.hMul (Real.exp (Real.instNeg.neg (instHMul.hMul lam t)))
                (MeasureTheory.integral μ fun ω => Real.exp (instHMul.hMul lam (S ω)))))
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst}
  [@MeasureTheory.IsProbabilityMeasure.{u_1} Ω inst μ] {S : Ω → Real}
  (hS : @Measurable.{u_1, 0} Ω Real inst Real.measurableSpace S) {lam t : Real}
  ...
  (hExp :
    @MeasureTheory.Integrable.{0, u_1} Real
      ...
      Ω inst (fun (ω : Ω) => Real.exp (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul) lam (S ω)))
      μ),
  And
    (@Eq.{1} Real
      (@MeasureTheory.Measure.real.{u_1} Ω inst μ
        (@Set.preimage.{u_1, 0} Ω Real S (@Set.Ici.{0} Real Real.instPreorder t)))
      (@MeasureTheory.Measure.real.{u_1} Ω inst μ
        (@Set.preimage.{u_1, 0} Ω Real
          (fun (ω : Ω) => Real.exp (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul) lam (S ω)))
          (@Set.Ici.{0} Real Real.instPreorder
            (Real.exp (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul) lam t))))))
    (@LE.le.{0} Real Real.instLE
      ...
      (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul)
        ...
        (@MeasureTheory.integral.{u_1, 0} Ω Real Real.normedAddCommGroup
          ...
          inst μ fun (ω : Ω) =>
          Real.exp (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul) lam (S ω)))))
```
...
### D010: `MeasureTheory.IsProbabilityMeasure`
...
### D012: `MeasureTheory.Measure.real`
...
### D018: `NormedCommRing.toSeminormedCommRing`
...
### D035: `SeminormedAddCommGroup.toSeminormedAddGroup`
...
### D036: `SeminormedAddGroup.toContinuousENorm`
...
### D037: `SeminormedCommRing.toNonUnitalSeminormedCommRing`
...
```lean
{α : Type u} → {β : Type v} → (f : α → β) → (s : Set.{v} β) → Set.{u} α
```
...
SHA-256: `b2516642cbc7b90008d4cbdcbf7008aca1307b4bc624d17bcc1813e8078ccb22`
