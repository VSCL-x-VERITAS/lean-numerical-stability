...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] {μ : MeasureTheory.Measure Ω} [MeasureTheory.IsProbabilityMeasure μ]
  {X : Ω → Real},
  Iff (ENNReal.instPartialOrder.lt (NumStability.HDP.Scalar.SubGaussian.PsiTwoGauge μ X) instTopENNReal.top)
  ...
  Iff
    ...
    (@Exists.{1} Real fun (K : Real) =>
      And (@LT.lt.{0} Real Real.instLT (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero)) K)
        (@NumStability.HDP.Scalar.SubGaussian.SubGaussianSquarePoint.{u_1} Ω inst μ X K))
```
...
### D002: `NumStability.HDP.Scalar.SubGaussian.SubGaussianSquarePoint`
...
### D034: `InnerProductSpace.toNormedSpace`
