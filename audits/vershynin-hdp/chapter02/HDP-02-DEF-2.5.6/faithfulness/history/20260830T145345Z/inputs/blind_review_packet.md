    ...
    And
      (∀ (i : LocalDef005),
        Ne i LocalDef006 →
          Iff (LocalDef001 μ X)
            (Exists fun K =>
              And (Real.instLT.lt 0 K) (LocalDef004 μ X i K)))
      (LocalDef001 μ X →
        And (ENNReal.instPartialOrder.lt (LocalDef003 μ X) instTopENNReal.top)
          (Eq (LocalDef003 μ X)
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst}
  ...
  And
    (∀ (i : LocalDef005),
      @Ne.{1} LocalDef005 i
          LocalDef006 →
        Iff (@LocalDef001.{u_1} Ω inst μ X)
          (@Exists.{1} Real fun (K : Real) =>
            And
              (@LT.lt.{0} Real Real.instLT (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero))
                K)
              (@LocalDef004.{u_1} Ω inst μ X i K)))
    (@LocalDef001.{u_1} Ω inst μ X →
      And
        (@LT.lt.{0} ENNReal (@Preorder.toLT.{0} ENNReal (@PartialOrder.toPreorder.{0} ENNReal ENNReal.instPartialOrder))
          (@LocalDef003.{u_1} Ω inst μ X) (@Top.top.{0} ENNReal instTopENNReal))
        (@Eq.{1} ENNReal (@LocalDef003.{u_1} Ω inst μ X)
...
fun {Ω} [MeasurableSpace Ω] μ X =>
  And (Measurable X)
    (Exists fun K => And (Real.instLT.lt 0 K) (LocalDef016 μ X K))
```
...
{Ω : Type u_1} →
  [inst : MeasurableSpace Ω] →
    MeasureTheory.Measure Ω → (Ω → Real) → LocalDef005 → Real → Prop
```
...
fun {Ω} [MeasurableSpace Ω] μ X x =>
  LocalDef011 (fun x => Real → Prop) x
    (fun _ => LocalDef018 μ X)
    (fun _ => LocalDef010 μ X)
    (fun _ => LocalDef017 μ X)
    (fun _ => LocalDef016 μ X) fun _ =>
    LocalDef009 μ X
```
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X K =>
  And (Measurable X) (And (Real.instLT.lt 0 K) (LocalDef019 μ X K))
```
...
```lean
(motive : LocalDef005 → Sort u_1) →
  (x : LocalDef005) →
    (Unit → motive LocalDef015) →
      (Unit → motive LocalDef012) →
        (Unit → motive LocalDef014) →
          (Unit → motive LocalDef013) →
            (Unit → motive LocalDef006) → motive x
```
...
fun motive x h_1 h_2 h_3 h_4 h_5 =>
  LocalDef020 x (h_1 Unit.unit) (h_2 Unit.unit) (h_3 Unit.unit)
...
```lean
{motive : LocalDef005 → Sort u} →
  (t : LocalDef005) →
    motive LocalDef015 →
      motive LocalDef012 →
        motive LocalDef014 →
          motive LocalDef013 →
            motive LocalDef006 → motive t
```
...
fun {motive} t tail moment squareWindow squarePoint linearMGF =>
  LocalDef021 tail moment squareWindow squarePoint linearMGF t
```
