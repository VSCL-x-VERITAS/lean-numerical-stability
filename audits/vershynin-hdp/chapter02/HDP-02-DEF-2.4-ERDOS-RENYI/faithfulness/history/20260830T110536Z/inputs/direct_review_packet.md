...
∀ (n : Nat) (p : (Set.Icc 0 1).Elem),
  And (Eq (NumStability.HDP.Contract.hdp_02_hdef_herdos_hrenyi n p).graphLaw (SimpleGraph.binomialRandom (Fin n) p))
    (∀ (v : Fin n) (G : SimpleGraph (Fin n)),
      Eq ((NumStability.HDP.Contract.hdp_02_hdef_herdos_hrenyi n p).degree v G) (G.degree v))
```
...
```lean
∀ {α : Sort u_4} [Finite.{u_4} α] {p : α → Prop}, Finite.{max 1 u_4} (@Subtype.{u_4} α fun (x : α) => p x)
```
