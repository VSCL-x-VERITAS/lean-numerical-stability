# Declaration dossier for HDP-02-EX-2.2.8
...
∀ {ι : Type u_1} {Ω : Type u_2} [inst : Fintype ι] [inst_1 : MeasurableSpace Ω] {μ : MeasureTheory.Measure Ω}
  [MeasureTheory.IsProbabilityMeasure μ] {W : ι → Ω → Real} {δ ε : Real},
  (∀ (i : ι), Measurable (W i)) →
    ProbabilityTheory.iIndepFun W μ →
      (∀ (i : ι), Filter.Eventually (fun ω => Set.instMembership.mem (Set.Icc 0 1) (W i ω)) (MeasureTheory.ae μ)) →
        (∀ (i : ι), Real.instLE.le (MeasureTheory.integral μ fun y => W i y) (instHSub.hSub (1 / 2) δ)) →
          Real.instLT.lt 0 δ →
            Real.instLT.lt 0 ε →
              Real.instLT.lt ε 1 →
                instLTNat.lt 0 (Fintype.card ι) →
                  Real.instLE.le (instHDiv.hDiv (Real.log (instHDiv.hDiv 1 ε)) (instHMul.hMul 2 (instHPow.hPow δ 2)))
                      (Fintype.card ι).cast →
                    Real.instLE.le
                      (μ.real
                        (setOf fun ω => GE.ge (Finset.univ.sum fun i => W i ω) (instHDiv.hDiv (Fintype.card ι).cast 2)))
                      ε
```
...
∀ {ι : Type u_1} {Ω : Type u_2} [inst : Fintype.{u_1} ι] [inst_1 : MeasurableSpace.{u_2} Ω]
  {μ : @MeasureTheory.Measure.{u_2} Ω inst_1} [@MeasureTheory.IsProbabilityMeasure.{u_2} Ω inst_1 μ] {W : ι → Ω → Real}
  {δ ε : Real} (hW : ∀ (i : ι), @Measurable.{u_2, 0} Ω Real inst_1 Real.measurableSpace (W i))
  (hIndep :
    @ProbabilityTheory.iIndepFun.{u_2, u_1, 0} Ω ι inst_1 (fun (x : ι) => Real) (fun (x : ι) => Real.measurableSpace) W
      μ)
  (hbound :
    ∀ (i : ι),
        ...
        (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul)
          (@OfNat.ofNat.{0} Real (nat_lit 2)
            (@instOfNatAtLeastTwo.{0} Real (nat_lit 2) Real.instNatCast
              (@Nat.instAtLeastTwoHAddOfNat (@OfNat.ofNat.{0} Nat (nat_lit 1) (instOfNatNat (nat_lit 1)))
                (@Nat.instNeZeroSucc (@OfNat.ofNat.{0} Nat (nat_lit 0) (instOfNatNat (nat_lit 0)))))))
          (@HPow.hPow.{0, 0, 0} Real Nat Real (@instHPow.{0, 0} Real Nat (@Monoid.toNatPow.{0} Real Real.instMonoid)) δ
    ...
    (@MeasureTheory.Measure.real.{u_2} Ω inst_1 μ
      (@setOf.{u_2} Ω fun (ω : Ω) =>
        @GE.ge.{0} Real Real.instLE
          (@Finset.sum.{u_1, 0} ι Real Real.instAddCommMonoid (@Finset.univ.{u_1} ι inst) fun (i : ι) => W i ω)
          (@HDiv.hDiv.{0, 0, 0} Real Real Real (@instHDiv.{0} Real (@DivInvMonoid.toDiv.{0} Real Real.instDivInvMonoid))
            (@Nat.cast.{0} Real Real.instNatCast (@Fintype.card.{u_1} ι inst))
            (@OfNat.ofNat.{0} Real (nat_lit 2)
              (@instOfNatAtLeastTwo.{0} Real (nat_lit 2) Real.instNatCast
...
### D020: `MeasureTheory.Measure.instOuterMeasureClass`
