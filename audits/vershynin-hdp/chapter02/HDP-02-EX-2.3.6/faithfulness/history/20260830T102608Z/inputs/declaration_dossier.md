          ...
          Real.instLT.lt 0 t →
            Real.instLE.le t rate.toReal →
              Real.instLE.le
                ...
                (instHMul.hMul 2
                  (Real.exp (Real.instNeg.neg (instHDiv.hDiv (instHMul.hMul c (instHPow.hPow t 2)) rate.toReal)))))
```
      ...
      @LT.lt.{0} NNReal (@Preorder.toLT.{0} NNReal (@PartialOrder.toPreorder.{0} NNReal instPartialOrderNNReal))
        ...
        ∀ {t : Real},
          @LT.lt.{0} Real Real.instLT (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero)) t →
            @LE.le.{0} Real Real.instLE t (NNReal.toReal rate) →
...
### D023: `ProbabilityTheory.poissonMeasure`
...
SHA-256: `bc71b1cf3f04a8323d59f264e89e2e6ef1432e43e62dd56c3c6cbb4f2e15f3df`
      ...
      _ = (Finset.univ.erase v).sum (fun _w => (p : ℝ)) := by
        calc
          ...
          _ = (Finset.univ.erase v).sum (fun _w => (p : ℝ)) := by
            ...
            intro w hw
            have hvw : v ≠ w := by
              exact Ne.symm (Finset.mem_erase.mp hw).1
            simp [hvw]
      _ = ((n - 1 : ℕ) : ℝ) * (p : ℝ) := by
