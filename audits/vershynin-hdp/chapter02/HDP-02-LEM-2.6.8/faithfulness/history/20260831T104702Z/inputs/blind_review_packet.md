            ...
            (And
              ...
              (And
                ...
                (@LE.le.{0} ENNReal
                  ...
                  (@LocalDef001.{u_1} Ω inst μ fun (ω : Ω) =>
                    @HSub.hSub.{0, 0, 0} Real Real Real (@instHSub.{0} Real Real.instSub) (X ω)
                      (@MeasureTheory.integral.{u_1, 0} Ω Real Real.normedAddCommGroup
                        (@InnerProductSpace.toNormedSpace.{0, 0} Real Real Real.instRCLike
                          (@NormedAddCommGroup.toSeminormedAddCommGroup.{0} Real Real.normedAddCommGroup)
...
{Ω : Type u_1} →
  [inst : MeasurableSpace Ω] →
    MeasureTheory.Measure Ω → (Ω → Real) → LocalDef003 → Real → Prop
```
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X K =>
  And (Measurable X) (And (Real.instLT.lt 0 K) (LocalDef017 μ X K))
```
  ...
  (x : LocalDef003) →
    (Unit → motive LocalDef012) →
      (Unit → motive LocalDef010) →
        (Unit → motive LocalDef011) →
          (Unit → motive LocalDef004) →
            (Unit → motive LocalDef009) → motive x
```
...
fun motive x h_1 h_2 h_3 h_4 h_5 =>
  LocalDef018 x (h_1 Unit.unit) (h_2 Unit.unit) (h_3 Unit.unit)
