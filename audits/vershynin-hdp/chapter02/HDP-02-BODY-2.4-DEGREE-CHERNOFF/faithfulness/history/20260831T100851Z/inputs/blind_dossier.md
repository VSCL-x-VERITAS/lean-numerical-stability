...
And
  (∀ (n : Nat) (p : (Set.Icc 0 1).Elem) (v : Fin n),
    ProbabilityTheory.HasLaw ((LocalDef003 n p).degree v)
      (LocalDef004 n p)
      (LocalDef003 n p).graphLaw)
  (Exists fun c =>
...
{n : Nat} →
  {p : (Set.Icc 0 1).Elem} →
    LocalDef005 n p → Fin n → SimpleGraph (Fin n) → Nat
```
...
```lean
(n : Nat) → (p : (Set.Icc 0 1).Elem) → LocalDef005 n p
```
...
{n : Nat} →
  {p : (Set.Icc 0 1).Elem} →
    MeasureTheory.Measure (SimpleGraph (Fin n)) →
      (Fin n → SimpleGraph (Fin n) → Nat) → LocalDef005 n p
```
