    ...
    (∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst}
      ...
      @Measurable.{u_1, 0} Ω Real inst Real.measurableSpace X →
        @LT.lt.{0} ENNReal (@Preorder.toLT.{0} ENNReal (@PartialOrder.toPreorder.{0} ENNReal ENNReal.instPartialOrder))
          ...
          And (@LocalDef002.{u_1} Ω inst μ inst_1 X)
            (And
              (∀ (i : LocalDef004),
                Or
                    (@Eq.{1} LocalDef004 i
                  ...
                  ∀ {K : Real},
                    @LT.lt.{0} Real Real.instLT
                      ...
                      @LocalDef003.{u_1} Ω inst μ X i K →
                        @LE.le.{0} ENNReal
...
fun {Ω} [MeasurableSpace Ω] μ [MeasureTheory.IsProbabilityMeasure μ] X =>
  And
    ...
    (And
      (LocalDef009 μ X
        (instHMul.hMul (instHMul.hMul 16 (Real.exp 1)) (LocalDef001 μ X).toReal))
      (And
        (And
...
fun {Ω} [MeasurableSpace Ω] μ X x =>
  LocalDef016 (fun x => Real → Prop) x
    ...
    (fun _ => LocalDef019 μ X)
    (fun _ => LocalDef018 μ X) fun _ =>
    LocalDef014 μ X
```
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X K =>
  And (Measurable X) (And (Real.instLT.lt 0 K) (LocalDef009 μ X K))
```
...
fun motive x h_1 h_2 h_3 h_4 h_5 =>
  LocalDef022 x (h_1 Unit.unit) (h_2 Unit.unit) (h_3 Unit.unit)
...
fun {motive} t tail moment squareWindow squarePoint linearMGF =>
  LocalDef023 tail moment squareWindow squarePoint linearMGF t
```
