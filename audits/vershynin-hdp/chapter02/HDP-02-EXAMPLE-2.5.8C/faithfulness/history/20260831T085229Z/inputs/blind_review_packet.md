...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] {μ : MeasureTheory.Measure Ω} [MeasureTheory.IsProbabilityMeasure μ]
  ...
  Measurable X →
    Real.instLT.lt 0 B →
      Filter.Eventually (fun ω => Real.instLE.le (abs (X ω)) B) (MeasureTheory.ae μ) →
        ENNReal.instPartialOrder.le (LocalDef001 μ X)
          (ENNReal.ofReal (instHDiv.hDiv B (Real.log 2).sqrt))
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst}
  [@MeasureTheory.IsProbabilityMeasure.{u_1} Ω inst μ] {X : Ω → Real} {B : Real}
  ...
  (hBound :
    @Filter.Eventually.{u_1} Ω
      (fun (ω : Ω) => @LE.le.{0} Real Real.instLE (@abs.{0} Real Real.lattice Real.instAddGroup (X ω)) B)
...
### D041: `CompleteLinearOrder.toConditionallyCompleteLinearOrderBot`
...
### D042: `ConditionallyCompleteLattice.toConditionallyCompletePartialOrder`
...
### D043: `ConditionallyCompleteLinearOrder.toConditionallyCompleteLattice`
...
### D044: `ConditionallyCompleteLinearOrderBot.toConditionallyCompleteLinearOrder`
...
### D045: `ConditionallyCompletePartialOrder.toConditionallyCompletePartialOrderInf`
...
### D046: `ConditionallyCompletePartialOrderInf.toInfSet`
...
### D047: `ENNReal.instCompleteLinearOrder`
