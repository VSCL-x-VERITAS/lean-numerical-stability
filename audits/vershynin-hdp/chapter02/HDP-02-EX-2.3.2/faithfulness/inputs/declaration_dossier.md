# Declaration dossier for HDP-02-EX-2.3.2
  ...
  ProbabilityTheory.iIndepFun B μ →
    (∀ (i : ι), ProbabilityTheory.HasLaw (B i) (PMF.bernoulli (p i) ⋯).toMeasure μ) →
      (∀ (i : ι), Measurable (B i)) →
        ∀ {t : Real},
          Real.instLT.lt 0 t →
            Real.instLT.lt t (Finset.univ.sum fun i => (p i).toReal) →
              Real.instLE.le
                (μ.real (setOf fun ω => Real.instLE.le (Finset.univ.sum fun i => ite (Eq (B i ω) Bool.true) 1 0) t))
                (instHMul.hMul (Real.exp (Real.instNeg.neg (Finset.univ.sum fun i => (p i).toReal)))
                  (instHPow.hPow (instHDiv.hDiv (instHMul.hMul (Real.exp 1) (Finset.univ.sum fun i => (p i).toReal)) t)
                    t))
```
...
∀ {ι : Type u_1} {Ω : Type u_2} [inst : Fintype.{u_1} ι] [inst_1 : MeasurableSpace.{u_2} Ω]
  ...
  (hp :
    ∀ (i : ι),
      @LE.le.{0} NNReal (@Preorder.toLE.{0} NNReal (@PartialOrder.toPreorder.{0} NNReal instPartialOrderNNReal)) (p i)
  ...
  (hMeas : ∀ (i : ι), @Measurable.{u_2, 0} Ω Bool inst_1 Bool.instMeasurableSpace (B i)) {t : Real}
