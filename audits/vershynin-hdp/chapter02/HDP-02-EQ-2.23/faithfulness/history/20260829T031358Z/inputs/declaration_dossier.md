...
theorem hdp_02_heq_h2_d23
    ...
    {lam t : ℝ} (hlam : 0 < lam)
    (hExp : ∀ i, Integrable (fun ω ↦ Real.exp (lam * X i ω)) μ) :
    μ.real {ω | ∑ i, X i ω ≥ t} ≤
      Real.exp (-(lam * t)) *
      ...
      ∀ {lam t : Real},
        Real.instLT.lt 0 lam →
          (∀ (i : ι), MeasureTheory.Integrable (fun ω => Real.exp (instHMul.hMul lam (X i ω))) μ) →
            Real.instLE.le (μ.real (setOf fun ω => GE.ge (Finset.univ.sum fun i => X i ω) t))
              (instHMul.hMul (Real.exp (Real.instNeg.neg (instHMul.hMul lam t)))
                (Finset.univ.prod fun i => MeasureTheory.integral μ fun ω => Real.exp (instHMul.hMul lam (X i ω))))
```
  ...
  (hExp :
    ∀ (i : ι),
      @MeasureTheory.Integrable.{0, u_2} Real
        ...
        Ω inst_1
        (fun (ω : Ω) => Real.exp (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul) lam (X i ω)))
        μ),
