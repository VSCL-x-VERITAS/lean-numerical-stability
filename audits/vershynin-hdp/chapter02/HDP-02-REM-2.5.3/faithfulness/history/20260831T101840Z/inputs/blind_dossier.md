                ...
                @Ne.{1} LocalDef001 j
                  ...
                  ∀ {Ki : Real},
                    @LT.lt.{0} Real Real.instLT
                      ...
                      @LocalDef003.{u_1} Ω inst μ X A i Ki →
            ...
            And
              ...
              ∀ (i j : LocalDef001) {Ki : Real},
                @LT.lt.{0} Real Real.instLT (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero))
                    Ki →
                  @LocalDef003.{u_2} Ω inst μ X A i Ki →
                    @Exists.{1} Real fun (Kj : Real) =>
                      And
                        (@LT.lt.{0} Real Real.instLT
                          (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero)) Kj)
                        (And
                          (@LE.le.{0} Real Real.instLE Kj
                            (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul) C Ki))
                          (@LocalDef003.{u_2} Ω inst μ X A j
                            Kj))))
```
...
{Ω : Type u_1} →
  [inst : MeasurableSpace Ω] →
    MeasureTheory.Measure Ω →
      (Ω → Real) → Real → LocalDef001 → Real → Prop
```
...
fun {Ω} [MeasurableSpace Ω] μ X A x =>
  LocalDef006 (fun x => Real → Prop) x
    (fun _ K => LocalDef013 μ X K A)
    (fun _ => LocalDef005 μ X)
    (fun _ => LocalDef012 μ X)
    (fun _ K => LocalDef011 μ X K A) fun _ =>
    LocalDef004 μ X
```
...
```lean
(motive : LocalDef001 → Sort u_1) →
  (x : LocalDef001) →
    (Unit → motive LocalDef010) →
      (Unit → motive LocalDef007) →
        (Unit → motive LocalDef009) →
          (Unit → motive LocalDef008) →
            (Unit → motive LocalDef002) → motive x
```
...
fun motive x h_1 h_2 h_3 h_4 h_5 =>
  LocalDef015 x (h_1 Unit.unit) (h_2 Unit.unit) (h_3 Unit.unit)
...
```lean
{motive : LocalDef001 → Sort u} →
  (t : LocalDef001) →
    motive LocalDef010 →
      motive LocalDef007 →
        motive LocalDef009 →
          motive LocalDef008 →
            motive LocalDef002 → motive t
```
...
fun {motive} t tail moment squareWindow squarePoint linearMGF =>
  LocalDef016 tail moment squareWindow squarePoint linearMGF t
```
