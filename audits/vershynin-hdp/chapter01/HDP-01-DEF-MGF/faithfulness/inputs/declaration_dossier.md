# Declaration dossier for HDP-01-DEF-MGF
...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] (μ : MeasureTheory.Measure Ω) [MeasureTheory.IsProbabilityMeasure μ]
  ...
  Measurable X →
    ∀ (t : Real),
      Eq (NumStability.HDP.Scalar.Preliminaries.mgf μ X t)
        (MeasureTheory.lintegral μ fun ω => ENNReal.ofReal (Real.exp (instHMul.hMul t (X ω))))
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] (μ : @MeasureTheory.Measure.{u_1} Ω inst)
  ...
  (_hX : @Measurable.{u_1, 0} Ω Real inst Real.measurableSpace X) (t : Real),
  @Eq.{1} ENNReal (@NumStability.HDP.Scalar.Preliminaries.mgf.{u_1} Ω inst μ X t)
    (@MeasureTheory.lintegral.{u_1} Ω inst μ fun (ω : Ω) =>
      ENNReal.ofReal (Real.exp (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul) t (X ω))))
```
...
### D001: `NumStability.HDP.Scalar.Preliminaries.mgf`

- Role: `local`
- Owner module: `NumStability.HDP.Scalar.Preliminaries`
- Declaration kind: `def`
- Distance from target type: `1`
- Semantic SHA-256: `2602930497c4e59786b472109e3aac9eb7331b803088a3babafdff5bce8fb15a`
...
```lean
{Ω : Type u_1} →
  [inst : MeasurableSpace.{u_1} Ω] → (μ : @MeasureTheory.Measure.{u_1} Ω inst) → (X : Ω → Real) → (t : Real) → ENNReal
```
...
fun {Ω} [MeasurableSpace Ω] μ X t =>
  MeasureTheory.lintegral μ fun ω => ENNReal.ofReal (NumStability.HDP.Scalar.Preliminaries.exponentialIntegrand X t ω)
```

### D002: `NumStability.HDP.Scalar.Preliminaries.exponentialIntegrand`

- Role: `local`
- Owner module: `NumStability.HDP.Scalar.Preliminaries`
- Declaration kind: `def`
- Distance from target type: `2`
- Semantic SHA-256: `83e36723ec944551da14613e235a7014c73a6c6adfeee22c6712580044ba22fa`

Type:

```lean
{α : Type u_1} → (α → Real) → Real → α → Real
```

Fully explicit type:

```lean
{α : Type u_1} → (X : α → Real) → (t : Real) → α → Real
```

Definition body (one-level semantic boundary):

```lean
fun {α} X t x => Real.exp (instHMul.hMul t (X x))
```
...
SHA-256: `f61eb1874d1e8bc670f5a8ca6349e18dbebd16ecb326f8d37bbf1501ad06358c`
