# Declaration dossier for HDP-01-EQ-TAIL-CDF
...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] {μ : MeasureTheory.Measure Ω} [MeasureTheory.IsProbabilityMeasure μ]
  ...
  AEMeasurable X μ →
    ∀ (t : Real),
      Eq (NumStability.HDP.Scalar.Preliminaries.upperTail μ X t)
        (instHSub.hSub 1 (NumStability.HDP.Scalar.Preliminaries.cdf μ X t))
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst}
  ...
  @Eq.{1} ENNReal (@NumStability.HDP.Scalar.Preliminaries.upperTail.{u_1} Ω inst μ X t)
    (@HSub.hSub.{0, 0, 0} ENNReal ENNReal ENNReal (@instHSub.{0} ENNReal ENNReal.instSub)
      ...
      (@NumStability.HDP.Scalar.Preliminaries.cdf.{u_1} Ω inst μ X t))
```
...
### D001: `NumStability.HDP.Scalar.Preliminaries.cdf`
...
fun {Ω} [MeasurableSpace Ω] μ X t =>
  MeasureTheory.Measure.instFunLike.coe (NumStability.HDP.Scalar.Preliminaries.distribution μ X) (Set.Iic t)
```

### D002: `NumStability.HDP.Scalar.Preliminaries.upperTail`

- Role: `local`
- Owner module: `NumStability.HDP.Scalar.Preliminaries`
- Declaration kind: `def`
- Distance from target type: `1`
- Semantic SHA-256: `745b75aa69f883c4a3049faf88b88d4c190d23dc906015fcbadde28de55c6b0e`
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X t =>
  MeasureTheory.Measure.instFunLike.coe (NumStability.HDP.Scalar.Preliminaries.distribution μ X) (Set.Ioi t)
```

### D003: `NumStability.HDP.Scalar.Preliminaries.distribution`
...
{Ω : Type u_1} →
  [inst : MeasurableSpace.{u_1} Ω] →
    (μ : @MeasureTheory.Measure.{u_1} Ω inst) → (X : Ω → Real) → @MeasureTheory.Measure.{0} Real Real.measurableSpace
```
...
### D018: `instAddCommMonoidWithOneENNReal`
...
### D026: `MeasureTheory.Measure.map`
