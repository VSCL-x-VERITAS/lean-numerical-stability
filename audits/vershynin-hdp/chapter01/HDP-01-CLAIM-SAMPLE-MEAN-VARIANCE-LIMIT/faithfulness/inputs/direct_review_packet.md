# Declaration dossier for HDP-01-CLAIM-SAMPLE-MEAN-VARIANCE-LIMIT
        ...
        Filter.Tendsto
          (fun N =>
            ProbabilityTheory.variance
              (fun ω => instHMul.hMul (Real.instInv.inv (instHAdd.hAdd N 1).cast) (Finset.univ.sum fun i => X i.val ω))
              μ)
          Filter.atTop (nhds 0)
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst}
  [@MeasureTheory.IsFiniteMeasure.{u_1} Ω inst μ] {X : Nat → Ω → Real}
  (hX :
    ∀ (i : Nat),
...
### D016: `MeasureTheory.IsFiniteMeasure`
...
### D029: `ProbabilityTheory.IdentDistrib`
...
### D030: `ProbabilityTheory.IndepFun`
