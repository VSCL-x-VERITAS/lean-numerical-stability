...
∀ (p : NNReal) (hp : instPartialOrderNNReal.le p 1),
  And (Eq (PMF.instFunLike.coe (LocalDef001 p hp) 1) (ENNReal.ofNNReal p))
    (And
      (Eq (PMF.instFunLike.coe (LocalDef001 p hp) 0)
        (instHSub.hSub 1 (ENNReal.ofNNReal p)))
      (And
        (∀ (k : Nat),
          Ne k 0 → Ne k 1 → Eq (PMF.instFunLike.coe (LocalDef001 p hp) k) 0)
        (And
          (Eq
            (MeasureTheory.integral (LocalDef002 p hp).toMeasure fun x => x)
            p.toReal)
          (Eq
            (MeasureTheory.integral (LocalDef002 p hp).toMeasure fun x =>
