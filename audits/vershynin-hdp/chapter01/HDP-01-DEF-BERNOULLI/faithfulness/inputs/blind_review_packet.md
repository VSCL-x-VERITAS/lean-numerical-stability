...
∀ (p : NNReal),
  instPartialOrderNNReal.lt 0 p →
    ∀ (hp1 : instPartialOrderNNReal.lt p 1),
      And (Eq (PMF.instFunLike.coe (LocalDef001 p ⋯) 1) (ENNReal.ofNNReal p))
        (And
          (Eq (PMF.instFunLike.coe (LocalDef001 p ⋯) 0)
            (instHSub.hSub 1 (ENNReal.ofNNReal p)))
          (And
            (∀ (k : Nat),
              Ne k 0 →
                Ne k 1 → Eq (PMF.instFunLike.coe (LocalDef001 p ⋯) k) 0)
            (And
              (Eq
                (MeasureTheory.integral (LocalDef002 p ⋯).toMeasure fun x =>
                  x)
                p.toReal)
              (Eq
                (MeasureTheory.integral (LocalDef002 p ⋯).toMeasure fun x =>
