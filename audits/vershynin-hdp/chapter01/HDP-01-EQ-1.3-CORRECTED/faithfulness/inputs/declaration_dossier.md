# Declaration dossier for HDP-01-EQ-1.3-CORRECTED
...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] {μ : MeasureTheory.Measure Ω} [MeasureTheory.IsProbabilityMeasure μ]
  {X : Ω → Real} {p q : ENNReal},
  Ne p 0 →
    ENNReal.instPartialOrder.le p q →
      MeasureTheory.AEStronglyMeasurable X μ →
        ENNReal.instPartialOrder.le (MeasureTheory.eLpNorm X p μ) (MeasureTheory.eLpNorm X q μ)
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst}
  [@MeasureTheory.IsProbabilityMeasure.{u_1} Ω inst μ] {X : Ω → Real} {p q : ENNReal}
  (_hp : @Ne.{1} ENNReal p (@OfNat.ofNat.{0} ENNReal (nat_lit 0) (@Zero.toOfNat0.{0} ENNReal instZeroENNReal)))
  (hpq :
    @LE.le.{0} ENNReal (@Preorder.toLE.{0} ENNReal (@PartialOrder.toPreorder.{0} ENNReal ENNReal.instPartialOrder)) p q)
  (hX :
...
### D006: `MeasureTheory.AEStronglyMeasurable`
...
### D021: `SeminormedAddCommGroup.toSeminormedAddGroup`
...
### D022: `SeminormedAddGroup.toContinuousENorm`
...
### D023: `SeminormedAddGroup.toPseudoMetricSpace`
...
### D024: `SeminormedCommRing.toNonUnitalSeminormedCommRing`
...
### D025: `UniformSpace.toTopologicalSpace`
