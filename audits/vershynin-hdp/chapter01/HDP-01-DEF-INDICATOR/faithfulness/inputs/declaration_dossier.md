# Declaration dossier for HDP-01-DEF-INDICATOR
...
theorem hdp_01_hdef_hindicator
    ...
    (ω ∈ E → indicatorFunction E ω = 1) ∧
      (ω ∉ E → indicatorFunction E ω = 0)
```

## Elaborated target type

```lean
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] (E : Set Ω) (ω : Ω),
  And (Set.instMembership.mem E ω → Eq (NumStability.HDP.Scalar.Preliminaries.indicatorFunction E ω) 1)
    (Not (Set.instMembership.mem E ω) → Eq (NumStability.HDP.Scalar.Preliminaries.indicatorFunction E ω) 0)
```

## Fully explicit elaborated target type

```lean
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] (E : Set.{u_1} Ω) (ω : Ω),
  And
    (@Membership.mem.{u_1, u_1} Ω (Set.{u_1} Ω) (@Set.instMembership.{u_1} Ω) E ω →
      @Eq.{1} Real (@NumStability.HDP.Scalar.Preliminaries.indicatorFunction.{u_1} Ω inst E ω)
        (@OfNat.ofNat.{0} Real (nat_lit 1) (@One.toOfNat1.{0} Real Real.instOne)))
    (Not (@Membership.mem.{u_1, u_1} Ω (Set.{u_1} Ω) (@Set.instMembership.{u_1} Ω) E ω) →
      @Eq.{1} Real (@NumStability.HDP.Scalar.Preliminaries.indicatorFunction.{u_1} Ω inst E ω)
...
### D001: `NumStability.HDP.Scalar.Preliminaries.indicatorFunction`
