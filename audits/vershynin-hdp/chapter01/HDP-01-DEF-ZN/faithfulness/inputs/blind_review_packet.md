...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] (μ : MeasureTheory.Measure Ω) (X : Nat → Ω → Real) (m σ : Real) (N : Nat),
  instLTNat.lt 0 N →
    Real.instLT.lt 0 σ →
      Eq (LocalDef002 μ fun ω => (Finset.range N).sum fun i => X i ω)
          (instHMul.hMul N.cast m) →
        Eq (LocalDef003 μ fun ω => (Finset.range N).sum fun i => X i ω)
            (instHMul.hMul N.cast (instHPow.hPow σ 2)) →
          Eq
            (fun ω =>
              instHDiv.hDiv
                (instHSub.hSub ((Finset.range N).sum fun i => X i ω)
                  (LocalDef002 μ fun ω => (Finset.range N).sum fun i => X i ω))
                (LocalDef003 μ fun ω => (Finset.range N).sum fun i => X i ω).sqrt)
            (LocalDef001 X m σ N)
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] (μ : @MeasureTheory.Measure.{u_1} Ω inst) (X : Nat → Ω → Real)
  ...
  (hVar :
    @Eq.{1} Real
      (@LocalDef003.{u_1} Ω inst μ fun (ω : Ω) =>
        @Finset.sum.{0, 0} Nat Real Real.instAddCommMonoid (Finset.range N) fun (i : Nat) => X i ω)
...
fun {Ω} [MeasurableSpace Ω] μ X =>
  LocalDef002 μ fun ω =>
    instHPow.hPow (instHSub.hSub (X ω) (LocalDef002 μ X)) 2
```
