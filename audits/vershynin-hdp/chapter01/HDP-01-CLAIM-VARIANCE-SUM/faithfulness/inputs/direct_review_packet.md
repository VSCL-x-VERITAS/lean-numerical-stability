# Declaration dossier for HDP-01-CLAIM-VARIANCE-SUM
...
∀ {ι : Type u_1} {Ω : Type u_2} [inst : Fintype ι] [inst_1 : MeasurableSpace Ω] {μ : MeasureTheory.Measure Ω}
  [MeasureTheory.IsFiniteMeasure μ] {X : ι → Ω → Real},
  (∀ (i : ι), MeasureTheory.MemLp (X i) 2 μ) →
    (∀ ⦃i j : ι⦄, Ne i j → ProbabilityTheory.IndepFun (X i) (X j) μ) →
      Eq (ProbabilityTheory.variance (Finset.univ.sum fun i => X i) μ)
...
∀ {ι : Type u_1} {Ω : Type u_2} [inst : Fintype.{u_1} ι] [inst_1 : MeasurableSpace.{u_2} Ω]
  {μ : @MeasureTheory.Measure.{u_2} Ω inst_1} [@MeasureTheory.IsFiniteMeasure.{u_2} Ω inst_1 μ] {X : ι → Ω → Real}
  (hX :
    ∀ (i : ι),
      @MeasureTheory.MemLp.{u_2, 0} Ω Real inst_1
  ...
  (hIndep :
    ∀ ⦃i j : ι⦄,
      @Ne.{u_1 + 1} ι i j →
        @ProbabilityTheory.IndepFun.{u_2, 0, 0} Ω Real Real inst_1 Real.measurableSpace Real.measurableSpace (X i) (X j)
          μ),
  @Eq.{1} Real
    (@ProbabilityTheory.variance.{u_2} Ω inst_1
      (@Finset.sum.{u_1, u_2} ι (Ω → Real)
        (@Pi.addCommMonoid.{u_2, 0} Ω (fun (a : Ω) => Real) fun (i : Ω) => Real.instAddCommMonoid)
        (@Finset.univ.{u_1} ι inst) fun (i : ι) => X i)
      μ)
    (@Finset.sum.{u_1, 0} ι Real Real.instAddCommMonoid (@Finset.univ.{u_1} ι inst) fun (i : ι) =>
      @ProbabilityTheory.variance.{u_2} Ω inst_1 (X i) μ)
```
...
### D002: `AddMonoidWithOne.toNatCast`
...
### D010: `MeasureTheory.IsFiniteMeasure`

- Role: `external-frontier`
- Owner module: `Mathlib.MeasureTheory.Measure.Typeclasses.Finite`
- Declaration kind: `inductive`
- Distance from target type: `1`
- Semantic SHA-256: `2291c3381bfe945ae44a347f0ffd18c5a143f45fd74dfb7fd931bdfb5b0ca991`
...
### D014: `Nat.instAtLeastTwoHAddOfNat`
...
### D017: `NonUnitalSeminormedCommRing.toNonUnitalSeminormedRing`
...
### D018: `NonUnitalSeminormedRing.toSeminormedAddCommGroup`
...
### D022: `ProbabilityTheory.IndepFun`
...
### D023: `ProbabilityTheory.variance`
