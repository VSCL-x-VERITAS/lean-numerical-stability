# Declaration dossier for HDP-01-LEM-1.2.1-INDICATOR-EXPECTATION
    ...
    ∀ (t : Real),
      Eq
        (NumStability.HDP.Scalar.Preliminaries.expectation μ
          (NumStability.HDP.Scalar.Preliminaries.indicatorFunction (setOf fun ω => Real.instLT.lt t (X ω))))
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] (μ : @MeasureTheory.Measure.{u_1} Ω inst)
  ...
  (hX : @Measurable.{u_1, 0} Ω Real inst Real.measurableSpace X) (t : Real),
  @Eq.{1} Real
    (@NumStability.HDP.Scalar.Preliminaries.expectation.{u_1} Ω inst μ
      (@NumStability.HDP.Scalar.Preliminaries.indicatorFunction.{u_1} Ω inst
        (@setOf.{u_1} Ω fun (ω : Ω) => @LT.lt.{0} Real Real.instLT t (X ω))))
    (@MeasureTheory.Measure.real.{u_1} Ω inst μ (@setOf.{u_1} Ω fun (ω : Ω) => @LT.lt.{0} Real Real.instLT t (X ω)))
```
...
### D002: `NumStability.HDP.Scalar.Preliminaries.indicatorFunction`

- Role: `local`
- Owner module: `NumStability.HDP.Scalar.Preliminaries`
- Declaration kind: `def`
- Distance from target type: `1`
- Semantic SHA-256: `3b1f59c18988324c668ddf8cb780938be4d3dff045f34e9dda5ba839c4e207b4`

Type:

```lean
{Ω : Type u_1} → [MeasurableSpace Ω] → Set Ω → Ω → Real
```

Fully explicit type:

```lean
{Ω : Type u_1} → [MeasurableSpace.{u_1} Ω] → (E : Set.{u_1} Ω) → Ω → Real
```

Definition body (one-level semantic boundary):

```lean
fun {Ω} [MeasurableSpace Ω] E => E.indicator fun x => 1
```
...
### D007: `MeasureTheory.IsProbabilityMeasure`
...
### D009: `MeasureTheory.Measure.real`
...
### D016: `NormedAddCommGroup.toSeminormedAddCommGroup`
...
```lean
{α : Type u_1} → {M : Type u_3} → [Zero.{u_3} M] → (s : Set.{u_1} α) → (f : α → M) → (x : α) → M
```
...
SHA-256: `c825829a6974079db4ea6acd7b281287322b67f5392b7417d3acd27fac88f1ba`
