...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] (μ : MeasureTheory.Measure Ω) [MeasureTheory.IsProbabilityMeasure μ]
  ...
  MeasureTheory.MemLp X 2 μ →
    And
      (Eq
        (LocalDef002 μ fun ω =>
          instHSub.hSub (X ω) (LocalDef001 μ X))
        (LocalDef004 μ X).sqrt)
      (Eq (LocalDef004 μ X).sqrt
        (LocalDef003 μ X))
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] (μ : @MeasureTheory.Measure.{u_1} Ω inst)
  ...
  And
    (@Eq.{1} Real
      ...
      (Real.sqrt (@LocalDef004.{u_1} Ω inst μ X)))
    (@Eq.{1} Real (Real.sqrt (@LocalDef004.{u_1} Ω inst μ X))
...
fun {Ω} [MeasurableSpace Ω] μ X =>
  (LocalDef001 μ fun ω => instHPow.hPow (X ω) 2).sqrt
```
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X => (LocalDef004 μ X).sqrt
```
...
fun {Ω} [MeasurableSpace Ω] μ X =>
  LocalDef001 μ fun ω =>
    instHPow.hPow (instHSub.hSub (X ω) (LocalDef001 μ X)) 2
```
