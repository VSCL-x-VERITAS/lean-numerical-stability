# Declaration dossier for HDP-01-THM-JENSEN
...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] {μ : MeasureTheory.Measure Ω} [MeasureTheory.IsProbabilityMeasure μ]
  {X : Ω → Real} {φ : Real → Real},
  ConvexOn Real Set.univ φ →
    MeasureTheory.Integrable X μ →
      MeasureTheory.Integrable (fun ω => φ (X ω)) μ →
        Real.instLE.le (φ (NumStability.HDP.Scalar.Preliminaries.expectation μ X))
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst}
  [@MeasureTheory.IsProbabilityMeasure.{u_1} Ω inst μ] {X : Ω → Real} {φ : Real → Real}
  (hφ :
        ...
        (@SeminormedAddCommGroup.toSeminormedAddGroup.{0} Real
          (@NonUnitalSeminormedRing.toSeminormedAddCommGroup.{0} Real
            (@NonUnitalSeminormedCommRing.toNonUnitalSeminormedRing.{0} Real
              (@SeminormedCommRing.toNonUnitalSeminormedCommRing.{0} Real
                (@NormedCommRing.toSeminormedCommRing.{0} Real Real.normedCommRing))))))
      Ω inst (fun (ω : Ω) => φ (X ω)) μ),
  @LE.le.{0} Real Real.instLE (φ (@NumStability.HDP.Scalar.Preliminaries.expectation.{u_1} Ω inst μ X))
    (@NumStability.HDP.Scalar.Preliminaries.expectation.{u_1} Ω inst μ fun (ω : Ω) => φ (X ω))
```
...
### D023: `SeminormedAddCommGroup.toSeminormedAddGroup`
...
### D024: `SeminormedAddGroup.toContinuousENorm`
...
### D025: `SeminormedCommRing.toNonUnitalSeminormedCommRing`
...
### `NumStability.HDP.ContractSignatures.C_01_hthm_hjensen`

Path: `lean-numerical-stability/NumStability/HDP/ContractSignatures/C_01_hthm_hjensen.lean`
