...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] (μ : MeasureTheory.Measure Ω)
  ...
  LocalDef001 μ X m σ hσ hX hIndep
    ...
    Filter.Tendsto
      (fun N =>
        MeasureTheory.ProbabilityMeasure.instFunLike.coe
          (LocalDef006
            (LocalDef005 X m σ (instHAdd.hAdd N 1)) ⋯)
          (Set.Iic t))
      Filter.atTop
      (nhds
        (MeasureTheory.ProbabilityMeasure.instFunLike.coe
          LocalDef004 (Set.Iic t)))
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] (μ : MeasureTheory.Measure Ω) (X : Nat → Ω → Real) (m σ : Real),
  (∀ (i : Nat), MeasureTheory.MemLp (X i) 2 μ) →
    ∀ (N : Nat), AEMeasurable (LocalDef005 X m σ (instHAdd.hAdd N 1)) μ
```
...
```lean
MeasureTheory.IsProbabilityMeasure LocalDef007
```
