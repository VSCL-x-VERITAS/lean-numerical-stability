# Declaration dossier for HDP-01-DEF-MOMENTS-CORRECTED
...
```lean
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] (μ : MeasureTheory.Measure Ω) (X : Ω → Real) (n : Nat) (p : Real),
  Real.instLT.lt 0 p →
    And
      (Eq (NumStability.HDP.Scalar.Preliminaries.rawMoment μ X n)
        (NumStability.HDP.Scalar.Preliminaries.expectation μ fun ω => instHPow.hPow (X ω) n))
      (Eq (NumStability.HDP.Scalar.Preliminaries.absoluteMoment μ X p)
        (MeasureTheory.lintegral μ fun ω => ENNReal.ofReal ((abs (X ω)).rpow p)))
```

## Fully explicit elaborated target type

```lean
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] (μ : @MeasureTheory.Measure.{u_1} Ω inst) (X : Ω → Real) (n : Nat)
  (p : Real)
  (_hp : @LT.lt.{0} Real Real.instLT (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero)) p),
  And
    (@Eq.{1} Real (@NumStability.HDP.Scalar.Preliminaries.rawMoment.{u_1} Ω inst μ X n)
      (@NumStability.HDP.Scalar.Preliminaries.expectation.{u_1} Ω inst μ fun (ω : Ω) =>
        @HPow.hPow.{0, 0, 0} Real Nat Real (@instHPow.{0, 0} Real Nat (@Monoid.toNatPow.{0} Real Real.instMonoid)) (X ω)
          n))
    (@Eq.{1} ENNReal (@NumStability.HDP.Scalar.Preliminaries.absoluteMoment.{u_1} Ω inst μ X p)
      (@MeasureTheory.lintegral.{u_1} Ω inst μ fun (ω : Ω) =>
        ENNReal.ofReal (Real.rpow (@abs.{0} Real Real.lattice Real.instAddGroup (X ω)) p)))
```
...
### D003: `NumStability.HDP.Scalar.Preliminaries.rawMoment`

- Role: `local`
- Owner module: `NumStability.HDP.Scalar.Preliminaries`
- Declaration kind: `def`
- Distance from target type: `1`
- Semantic SHA-256: `8dfef72ae41b5b1ceb63e9b34c13301b87d1a90e6df2d937272573268e9ce2d9`

Type:

```lean
{Ω : Type u_1} → [inst : MeasurableSpace Ω] → MeasureTheory.Measure Ω → (Ω → Real) → Nat → Real
```
...
```lean
{Ω : Type u_1} →
  [inst : MeasurableSpace.{u_1} Ω] → (μ : @MeasureTheory.Measure.{u_1} Ω inst) → (X : Ω → Real) → (n : Nat) → Real
```

Definition body (one-level semantic boundary):

```lean
fun {Ω} [MeasurableSpace Ω] μ X n => NumStability.HDP.Scalar.Preliminaries.expectation μ fun ω => instHPow.hPow (X ω) n
```
...
### D026: `InnerProductSpace.toNormedSpace`
...
### D028: `NormedAddCommGroup.toSeminormedAddCommGroup`
...
SHA-256: `94765ab6804b94b99df6188eb067d1e11ea7a4e8e2dd152ef26d21cfa6755d26`
