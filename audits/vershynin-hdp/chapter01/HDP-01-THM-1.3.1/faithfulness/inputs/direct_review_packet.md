# Declaration dossier for HDP-01-THM-1.3.1
...
```lean
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] {μ : MeasureTheory.Measure Ω} {X : Nat → Ω → Real},
  MeasureTheory.Integrable (X 0) μ →
    Pairwise (Function.onFun (fun x1 x2 => ProbabilityTheory.IndepFun x1 x2 μ) X) →
      (∀ (i : Nat), ProbabilityTheory.IdentDistrib (X i) (X 0) μ μ) →
        Filter.Eventually
          (fun ω =>
            Filter.Tendsto (fun n => instHDiv.hDiv ((Finset.range n).sum fun i => X i ω) n.cast) Filter.atTop
              (nhds (MeasureTheory.integral μ fun ω => X 0 ω)))
          (MeasureTheory.ae μ)
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst} {X : Nat → Ω → Real}
  (hInt :
    @MeasureTheory.Integrable.{0, u_1} Real
      ...
      Ω inst (X (@OfNat.ofNat.{0} Nat (nat_lit 0) (instOfNatNat (nat_lit 0)))) μ)
  (hIndep :
    @Pairwise.{0} Nat
      (@Function.onFun.{1, max 1 (u_1 + 1), 1} Nat (Ω → Real) Prop
        (fun (x1 x2 : Ω → Real) =>
  ...
  @Filter.Eventually.{u_1} Ω
    ...
    (@MeasureTheory.ae.{u_1, u_1} Ω (@MeasureTheory.Measure.{u_1} Ω inst)
      (@MeasureTheory.Measure.instFunLike.{u_1} Ω inst) (@MeasureTheory.Measure.instOuterMeasureClass.{u_1} Ω inst) μ)
```
...
### D013: `MeasureTheory.Measure.instFunLike`
...
### D014: `MeasureTheory.Measure.instOuterMeasureClass`
...
### D026: `ProbabilityTheory.IdentDistrib`
...
### D027: `ProbabilityTheory.IndepFun`
