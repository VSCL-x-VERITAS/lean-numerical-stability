# Declaration dossier for HDP-01-REM-1.1.1
...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] {μ : MeasureTheory.Measure Ω} [MeasureTheory.IsProbabilityMeasure μ]
  ...
  MeasureTheory.MemLp X 2 μ →
    MeasureTheory.MemLp Y 2 μ →
      Real.instLE.le (Real.norm.norm (NumStability.HDP.Scalar.Preliminaries.covariance μ X Y))
        (instHMul.hMul (NumStability.HDP.Scalar.Preliminaries.standardDeviation μ X)
          (NumStability.HDP.Scalar.Preliminaries.standardDeviation μ Y))
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst}
          ...
          (@SeminormedAddCommGroup.toSeminormedAddGroup.{0} Real
            (@NonUnitalSeminormedRing.toSeminormedAddCommGroup.{0} Real
              (@NonUnitalSeminormedCommRing.toNonUnitalSeminormedRing.{0} Real
                (@SeminormedCommRing.toNonUnitalSeminormedCommRing.{0} Real
                  (@NormedCommRing.toSeminormedCommRing.{0} Real Real.normedCommRing)))))))
      (@UniformSpace.toTopologicalSpace.{0} Real (@PseudoMetricSpace.toUniformSpace.{0} Real Real.pseudoMetricSpace)) Y
      (@OfNat.ofNat.{0} ENNReal (nat_lit 2)
        (@instOfNatAtLeastTwo.{0} ENNReal (nat_lit 2)
          (@AddMonoidWithOne.toNatCast.{0} ENNReal
            (@AddCommMonoidWithOne.toAddMonoidWithOne.{0} ENNReal instAddCommMonoidWithOneENNReal))
  ...
  @LE.le.{0} Real Real.instLE
    (@Norm.norm.{0} Real Real.norm (@NumStability.HDP.Scalar.Preliminaries.covariance.{u_1} Ω inst μ X Y))
    (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul)
      (@NumStability.HDP.Scalar.Preliminaries.standardDeviation.{u_1} Ω inst μ X)
      (@NumStability.HDP.Scalar.Preliminaries.standardDeviation.{u_1} Ω inst μ Y))
```
...
### D001: `NumStability.HDP.Scalar.Preliminaries.covariance`

- Role: `local`
- Owner module: `AuditTarget`
- Declaration kind: `def`
- Distance from target type: `1`
- Semantic SHA-256: `1e2f9eb6002c5bbe1f9701bdbc391894f8c51fc10c7cef9e8c11acd664f3758b`

Type:

```lean
{Ω : Type u_1} → [inst : MeasurableSpace Ω] → MeasureTheory.Measure Ω → (Ω → Real) → (Ω → Real) → Real
```

Fully explicit type:

```lean
{Ω : Type u_1} → [inst : MeasurableSpace.{u_1} Ω] → (μ : @MeasureTheory.Measure.{u_1} Ω inst) → (X Y : Ω → Real) → Real
```

Definition body (one-level semantic boundary):

```lean
fun {Ω} [MeasurableSpace Ω] μ X Y =>
  NumStability.HDP.Scalar.Preliminaries.expectation μ fun ω =>
    instHMul.hMul (instHSub.hSub (X ω) (NumStability.HDP.Scalar.Preliminaries.expectation μ X))
      (instHSub.hSub (Y ω) (NumStability.HDP.Scalar.Preliminaries.expectation μ Y))
```

### D002: `NumStability.HDP.Scalar.Preliminaries.standardDeviation`

- Role: `local`
- Owner module: `AuditTarget`
- Declaration kind: `def`
- Distance from target type: `1`
- Semantic SHA-256: `be1cb192ac57d54341b68b97814df19e29483573fd1b285d34ed02ebcfd40bba`
...
```lean
fun {Ω} [MeasurableSpace Ω] μ X => (NumStability.HDP.Scalar.Preliminaries.variance μ X).sqrt
```

### D003: `NumStability.HDP.Scalar.Preliminaries.expectation`

- Role: `local`
- Owner module: `AuditTarget`
- Declaration kind: `def`
- Distance from target type: `2`
- Semantic SHA-256: `3add2dba93b6f6f0d092ea352b0441c138012c46c6eb97f6753318bb38ffd9ac`
...
### D004: `NumStability.HDP.Scalar.Preliminaries.variance`

- Role: `local`
- Owner module: `AuditTarget`
- Declaration kind: `def`
- Distance from target type: `2`
- Semantic SHA-256: `1fdcc4512a4409c3f12ebf7456f29166de989e0b179449dc01723d642588ac2d`
...
### D030: `SeminormedAddCommGroup.toSeminormedAddGroup`
...
### D031: `SeminormedAddGroup.toContinuousENorm`
...
### D032: `SeminormedAddGroup.toPseudoMetricSpace`
...
### D035: `instAddCommMonoidWithOneENNReal`
...
### D044: `InnerProductSpace.toNormedSpace`
