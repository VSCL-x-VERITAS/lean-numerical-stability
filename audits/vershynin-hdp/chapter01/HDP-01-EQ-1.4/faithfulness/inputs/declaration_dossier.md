# Declaration dossier for HDP-01-EQ-1.4
...
∀ {Ω : Type u_1} [inst : MeasurableSpace Ω] {μ : MeasureTheory.Measure Ω} [MeasureTheory.IsProbabilityMeasure μ]
  {X Y : Ω → Real} {p : ENNReal},
  ENNReal.instPartialOrder.le 1 p →
    MeasureTheory.MemLp X p μ →
      MeasureTheory.MemLp Y p μ →
        ENNReal.instPartialOrder.le (MeasureTheory.eLpNorm (instHAdd.hAdd X Y) p μ)
          (instHAdd.hAdd (MeasureTheory.eLpNorm X p μ) (MeasureTheory.eLpNorm Y p μ))
```
...
∀ {Ω : Type u_1} [inst : MeasurableSpace.{u_1} Ω] {μ : @MeasureTheory.Measure.{u_1} Ω inst}
  [@MeasureTheory.IsProbabilityMeasure.{u_1} Ω inst μ] {X Y : Ω → Real} {p : ENNReal}
  (hp :
  ...
  @LE.le.{0} ENNReal (@Preorder.toLE.{0} ENNReal (@PartialOrder.toPreorder.{0} ENNReal ENNReal.instPartialOrder))
    (@MeasureTheory.eLpNorm.{u_1, 0} Ω Real
      ...
      (@HAdd.hAdd.{u_1, u_1, u_1} (Ω → Real) (Ω → Real) (Ω → Real)
        (@instHAdd.{u_1} (Ω → Real) (@Pi.instAdd.{u_1, 0} Ω (fun (a : Ω) => Real) fun (i : Ω) => Real.instAdd)) X Y)
      p μ)
...
### D016: `NonAssocSemiring.toNonUnitalNonAssocSemiring`
...
### D017: `NonUnitalNonAssocSemiring.toDistrib`
...
### D031: `SeminormedAddCommGroup.toSeminormedAddGroup`
...
### D032: `SeminormedAddGroup.toContinuousENorm`
...
### D033: `SeminormedAddGroup.toPseudoMetricSpace`
...
### D034: `SeminormedCommRing.toNonUnitalSeminormedCommRing`
...
### D035: `Semiring.toNonAssocSemiring`
...
### D037: `instAddCommMonoidWithOneENNReal`
