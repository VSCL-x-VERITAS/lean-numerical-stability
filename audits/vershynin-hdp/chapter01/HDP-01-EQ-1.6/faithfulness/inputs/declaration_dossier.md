...
```lean
And
  (Eq NumStability.HDP.Contract.hdp_01_hdef_hstandard_hnormal
    (Real.measureSpace.volume.withDensity (ProbabilityTheory.gaussianPDF 0 1)))
...
And
  (@Eq.{1} (@MeasureTheory.Measure.{0} Real Real.measurableSpace)
    ...
    (@MeasureTheory.Measure.withDensity.{0} Real
      ...
      (ProbabilityTheory.gaussianPDF (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero))
...
### D001: `NumStability.HDP.Contract.hdp_01_hdef_hstandard_hnormal`

- Role: `local`
- Owner module: `AuditTarget`
- Declaration kind: `def`
- Distance from target type: `1`
- Semantic SHA-256: `646059f507c27426e2c9ad670738b6530f8c67746491d44b99ffde0e3a253f72`
...
### D002: `NumStability.HDP.Scalar.LimitTheorems.standardNormalLaw`
...
### D011: `MeasureTheory.Measure.withDensity`

- Role: `external-frontier`
- Owner module: `Mathlib.MeasureTheory.Measure.WithDensity`
- Declaration kind: `def`
- Distance from target type: `1`
- Semantic SHA-256: `05b35f2f13a91451412aa890ff3413c0c5616bc538e96974c18d8761b398bab6`
...
{α : Type u_1} →
  {m : MeasurableSpace.{u_1} α} →
    (μ : @MeasureTheory.Measure.{u_1} α m) → (f : α → ENNReal) → @MeasureTheory.Measure.{u_1} α m
```
...
```lean
fun {α} {m} μ f =>
  MeasureTheory.Measure.ofMeasurable (fun s x => MeasureTheory.lintegral (μ.restrict s) fun a => f a) ⋯ ⋯
```

### D012: `MeasureTheory.MeasureSpace.toMeasurableSpace`
...
### D013: `MeasureTheory.MeasureSpace.volume`
...
### D022: `ProbabilityTheory.gaussianPDF`

- Role: `external-frontier`
- Owner module: `Mathlib.Probability.Distributions.Gaussian.Real`
- Declaration kind: `def`
- Distance from target type: `1`
- Semantic SHA-256: `4bbfdeb79167320f476caa41402600a96cb5981c930d1d31eba413ed29800874`

Type:

```lean
Real → NNReal → Real → ENNReal
```

Fully explicit type:

```lean
(μ : Real) → (v : NNReal) → (x : Real) → ENNReal
```

Definition body (one-level semantic boundary):

```lean
fun μ v x => ENNReal.ofReal (ProbabilityTheory.gaussianPDFReal μ v x)
```

### D023: `ProbabilityTheory.gaussianPDFReal`
...
### D044: `ProbabilityTheory.gaussianReal`
