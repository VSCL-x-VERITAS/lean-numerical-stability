  ...
  Ne p 0 →
    ∀ (X : Ω → Real),
      MeasureTheory.AEStronglyMeasurable X μ →
        Iff ((LocalDef002 μ p).representativeMember X)
...
{Ω : Type u_1} →
  [inst : MeasurableSpace Ω] →
    {μ : MeasureTheory.Measure Ω} →
      {p : ENNReal} → LocalDef003 μ p → (Ω → Real) → Prop
```
...
{Ω : Type u_1} →
  [inst : MeasurableSpace Ω] →
    (μ : MeasureTheory.Measure Ω) → (p : ENNReal) → LocalDef003 μ p
```
