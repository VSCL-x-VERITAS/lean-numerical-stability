...
∀ {Model : Type u_1} {Cell : Type u_2} {Point : Type u_3} {Parameter : Type u_4} {Property : Type u_5}
  ...
  (grid : LocalDef001 Cell Point) (volumeMeasure : MeasureTheory.Measure Point)
          ...
          (∀ (cell : Cell),
            ...
            And
              ...
              (Exists fun assignedCellProperties =>
                ∀ (cell : Cell),
                  And
                    (LocalDef004 volumeMeasure (grid.cellRegion cell) materialParameters
                      (LocalDef005 volumeMeasure (grid.cellRegion cell) materialParameters))
                    (And
...
∀ {Model : Type u_1} {Cell : Type u_2} {Point : Type u_3} {Parameter : Type u_4} {Property : Type u_5}
  ...
  (grid : @LocalDef001.{u_2, u_3} Cell Point inst)
    ...
    ∀ (cell : Cell),
      @Ne.{1} ENNReal
        (@DFunLike.coe.{u_3 + 1, u_3 + 1, 1} (@MeasureTheory.Measure.{u_3} Point inst) (Set.{u_3} Point)
          ...
          (@LocalDef002.{u_2, u_3} Cell Point inst grid cell))
      ...
      @MeasureTheory.IntegrableOn.{u_3, u_4} Point Parameter inst
        ...
        (@SeminormedAddGroup.toContinuousENorm.{u_4} Parameter
          (@SeminormedAddCommGroup.toSeminormedAddGroup.{u_4} Parameter
            (@NormedAddCommGroup.toSeminormedAddCommGroup.{u_4} Parameter inst_1)))
        materialParameters (@LocalDef002.{u_2, u_3} Cell Point inst grid cell)
        volumeMeasure)
  (hheterogeneous :
    @Exists.{u_3 + 1} Point fun (point₁ : Point) =>
      And
        (@Membership.mem.{u_3, u_3} Point (Set.{u_3} Point) (@Set.instMembership.{u_3} Point)
          (@LocalDef003.{u_2, u_3} Cell Point inst grid) point₁)
        (@Exists.{u_3 + 1} Point fun (point₂ : Point) =>
          And
            (@Membership.mem.{u_3, u_3} Point (Set.{u_3} Point) (@Set.instMembership.{u_3} Point)
              (@LocalDef003.{u_2, u_3} Cell Point inst grid) point₂)
  ...
  (happropriate :
    ∀ (cell : Cell),
      isAppropriateForModel model volumeMeasure
        (@LocalDef002.{u_2, u_3} Cell Point inst grid cell) materialParameters
        (propertyFromAverage model
          (@LocalDef005.{u_3, u_4} Point Parameter inst inst_1 inst_2 volumeMeasure
            (@LocalDef002.{u_2, u_3} Cell Point inst grid cell)
            materialParameters))),
  And
    (@Exists.{u_3 + 1} Point fun (point₁ : Point) =>
    ...
    (@Exists.{max (u_2 + 1) (u_5 + 1)} (Cell → Property) fun (assignedCellProperties : Cell → Property) =>
      ∀ (cell : Cell),
        And
          (@LocalDef004.{u_3, u_4} Point Parameter inst inst_1 inst_2 volumeMeasure
            (@LocalDef002.{u_2, u_3} Cell Point inst grid cell) materialParameters
            (@LocalDef005.{u_3, u_4} Point Parameter inst inst_1 inst_2 volumeMeasure
              (@LocalDef002.{u_2, u_3} Cell Point inst grid cell)
              materialParameters))
...
```lean
{Cell : Type u_1} →
  {Point : Type u_2} → [inst : MeasurableSpace Point] → LocalDef001 Cell Point → Set Point
```
...
fun {Point} {E} [MeasurableSpace Point] [NormedAddCommGroup E] [NormedSpace Real E] μ region field average =>
  And (Ne (MeasureTheory.Measure.instFunLike.coe μ region) 0)
    (And (Ne (MeasureTheory.Measure.instFunLike.coe μ region) instTopENNReal.top)
      (And (MeasureTheory.IntegrableOn field region μ) (Eq average (LocalDef005 μ region field))))
```
