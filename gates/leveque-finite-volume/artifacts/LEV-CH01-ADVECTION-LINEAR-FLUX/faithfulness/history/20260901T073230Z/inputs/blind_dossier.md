...
∀ (q : Real → Real → Real) (speed x t : Real),
  LocalDef003 q speed x t →
    And
      (LocalDef004 (LocalDef005 q)
        (LocalDef002 (LocalDef001 speed)) x t)
      (∀ (state : Fin 1 → Real),
        Eq (LocalDef002 (LocalDef001 speed) state) fun x =>
...
```lean
∀ (q : Real → Real → Real) (speed x t : Real)
  (hadvection : LocalDef003 q speed x t),
  And
    (@LocalDef004 (@OfNat.ofNat.{0} Nat (nat_lit 1) (instOfNatNat (nat_lit 1)))
      (LocalDef005 q)
