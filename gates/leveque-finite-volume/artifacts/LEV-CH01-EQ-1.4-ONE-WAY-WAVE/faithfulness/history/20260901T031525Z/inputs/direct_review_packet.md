...
theorem leveque01_equation04_positiveSpeedOneWayWave
    (w : ℝ → ℝ → ℝ) (c x t : ℝ) (hc : 0 < c) :
    (leveque01_equation04_oneWayWaveAt w c x t ↔
      ∃ wt wx : ℝ,
        HasDerivAt (fun τ => w x τ) wt t ∧
          HasDerivAt (fun ξ => w ξ t) wx x ∧
            wt + c • wx = 0) ∧
      ∀ (profile : ℝ → ℝ) (y s : ℝ), 0 < s →
        y < y + c * s ∧
          travelingWave profile c (y + c * s) s = profile y
```
...
∀ (w : Real → Real → Real) (c x t : Real),
  Real.instLT.lt 0 c →
    And
      (Iff (NumStability.leveque01_equation04_oneWayWaveAt w c x t)
        (Exists fun wt =>
          Exists fun wx =>
            And (HasDerivAt (fun τ => w x τ) wt t)
              (And (HasDerivAt (fun ξ => w ξ t) wx x) (Eq (instHAdd.hAdd wt (instHSMul.hSMul c wx)) 0))))
      (∀ (profile : Real → Real) (y s : Real),
        Real.instLT.lt 0 s →
          And (Real.instLT.lt y (instHAdd.hAdd y (instHMul.hMul c s)))
            (Eq (NumStability.travelingWave profile c (instHAdd.hAdd y (instHMul.hMul c s)) s) (profile y)))
```
  ...
  And
    ...
    (∀ (profile : Real → Real) (y s : Real),
      @LT.lt.{0} Real Real.instLT (@OfNat.ofNat.{0} Real (nat_lit 0) (@Zero.toOfNat0.{0} Real Real.instZero)) s →
        And
          (@LT.lt.{0} Real Real.instLT y
            (@HAdd.hAdd.{0, 0, 0} Real Real Real (@instHAdd.{0} Real Real.instAdd) y
              (@HMul.hMul.{0, 0, 0} Real Real Real (@instHMul.{0} Real Real.instMul) c s)))
          (@Eq.{1} Real
...
### D001: `NumStability.leveque01_equation04_oneWayWaveAt`
...
### D021: `NonUnitalCommRing.toNonUnitalNonAssocCommRing`
...
### D022: `NonUnitalNonAssocCommRing.toNonUnitalNonAssocRing`
...
### D023: `NonUnitalNonAssocRing.toNonUnitalNonAssocSemiring`
...
### D024: `NonUnitalNormedCommRing.toNonUnitalCommRing`
...
### D027: `NontriviallyNormedField.toNormedField`
...
### D030: `NormedField.toNormedCommRing`
...
### D054: `instIsTopologicalRingReal`
...
### D066: `DistribMulAction.toDistribSMul`
...
### D067: `DistribSMul.toSMulZeroClass`
...
### D068: `ENormedAddCommMonoid.toESeminormedAddCommMonoid`
...
### D069: `ESeminormedAddCommMonoid.toAddCommMonoid`
...
### D070: `ESeminormedAddCommMonoid.toESeminormedAddMonoid`
...
### D071: `ESeminormedAddMonoid.toAddMonoid`
...
### D072: `Module.toDistribMulAction`
...
### D075: `NormedAddCommGroup.toAddCommGroup`
...
### D076: `NormedAddCommGroup.toENormedAddCommMonoid`
...
### D083: `SubNegZeroMonoid.toNegZeroClass`
...
### D084: `SubtractionCommMonoid.toSubtractionMonoid`
...
### D085: `SubtractionMonoid.toSubNegZeroMonoid`
...
### D086: `AddCommGroup.toAddCommMonoid`
...
### D090: `DivisionSemiring.toSemiring`
...
### D094: `Semifield.toDivisionSemiring`
