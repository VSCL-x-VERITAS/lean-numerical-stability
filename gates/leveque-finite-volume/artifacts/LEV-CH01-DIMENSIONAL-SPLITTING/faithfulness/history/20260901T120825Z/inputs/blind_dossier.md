...
```lean
∀ {Direction : Type u_1} {State : Type u_2} (gridGeometry : LocalDef004)
  (coordinateDirections : List Direction) (steps : List (LocalDef001 Direction State)),
  (∀ (direction : Direction),
    ...
    ∀ (state : State),
      And
        (Or (Eq gridGeometry LocalDef006)
          (Eq gridGeometry LocalDef005))
        (And
          (∀ (direction : Direction),
            List.instMembership.mem coordinateDirections direction →
              Exists fun step => And (List.instMembership.mem steps step) (Eq step.direction direction))
          (Eq (LocalDef007 steps state)
            (LocalDef008 (List.map (fun step => step.oneDimensionalSolve) steps) state)))
```
...
∀ {Direction : Type u_1} {State : Type u_2} (gridGeometry : LocalDef004)
  ...
  (steps : List.{max u_2 u_1} (LocalDef001.{u_1, u_2} Direction State))
  (hcoverage :
    ∀ (direction : Direction),
        ...
        @Exists.{(max u_1 u_2) + 1} (LocalDef001.{u_1, u_2} Direction State)
          ...
          And
            (@Membership.mem.{max u_1 u_2, max u_1 u_2}
              ...
              steps step)
            (@Eq.{u_1 + 1} Direction (@LocalDef002.{u_1, u_2} Direction State step)
              direction))
  (state : State),
  And
    (Or (@Eq.{1} LocalDef004 gridGeometry LocalDef006)
      (@Eq.{1} LocalDef004 gridGeometry
        LocalDef005))
    (And
      (∀ (direction : Direction),
        @Membership.mem.{u_1, u_1} Direction (List.{u_1} Direction) (@List.instMembership.{u_1} Direction)
      ...
      (@Eq.{u_2 + 1} State (@LocalDef007.{u_1, u_2} Direction State steps state)
        (@LocalDef008.{u_2} State
          (@List.map.{max u_1 u_2, u_2} (LocalDef001.{u_1, u_2} Direction State)
            (State → State)
            (fun (step : LocalDef001.{u_1, u_2} Direction State) =>
    ...
    Direction →
      (timeFraction : Real) →
        Real.instLT.lt 0 timeFraction →
          Real.instLE.le timeFraction 1 → (State → State) → LocalDef001 Direction State
```
