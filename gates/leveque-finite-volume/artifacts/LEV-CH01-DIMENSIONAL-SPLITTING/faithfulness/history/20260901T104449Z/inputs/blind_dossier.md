...
And
  (∀ {Direction : Type u_1} {Cell : Type u_2} {E : Type u_3} [inst : AddCommGroup E] [inst_1 : Module Real E]
    (grid : LocalDef001 Direction Cell)
    (methods : Direction → LocalDef009 Direction E)
    ...
    (∀ (direction : Direction), Eq (methods direction).direction direction) →
      directionOrder.Perm grid.coordinateDirections →
        And
          (Or (Eq grid.geometry LocalDef008)
            (Eq grid.geometry LocalDef007))
              ...
              (And
                ...
                (And
                  (∀ (direction : Direction) (cellAverages : Cell → E) (cell : Cell),
                    Eq (LocalDef015 grid (methods direction) cellAverages cell)
                  ...
                  (And
                    (∀ (direction : Direction) (state : E),
                      Eq (LocalDef015 grid (methods direction) fun x => state) fun x =>
                        state)
                    (∀ (cellAverages : Cell → E),
                      Eq (LocalDef016 grid methods directionOrder cellAverages)
                        (LocalDef020
                          (List.map
                            (fun direction => LocalDef015 grid (methods direction))
                            directionOrder)
                          cellAverages))))))))
  (have splitAdvance :=
    LocalDef016 LocalDef017
      LocalDef018 (List.cons Bool.false (List.cons Bool.true List.nil));
  have diagonalReference := fun cellAverages cell =>
    cellAverages { fst := instHSub.hSub cell.fst 1, snd := instHSub.hSub cell.snd 1 };
  And (LocalDef014 splitAdvance diagonalReference Set.univ)
    (LocalDef013 splitAdvance LocalDef019 fun cell =>
...
And
  (∀ {Direction : Type u_1} {Cell : Type u_2} {E : Type u_3} [inst : AddCommGroup.{u_3} E]
    ...
    (grid : LocalDef001.{u_1, u_2} Direction Cell)
    (methods : Direction → LocalDef009.{u_1, u_3} Direction E)
    (directionOrder : List.{u_1} Direction)
    (hmethodDirection :
      ∀ (direction : Direction),
        @Eq.{u_1 + 1} Direction
          (@LocalDef010.{u_1, u_3} Direction E
            (methods direction))
          direction)
    (horder :
      @List.Perm.{u_1} Direction directionOrder
        (@LocalDef002.{u_1, u_2} Direction Cell grid)),
    And
      (Or
        (@Eq.{1} LocalDef006
          (@LocalDef003.{u_1, u_2} Direction Cell grid)
          LocalDef008)
        (@Eq.{1} LocalDef006
          (@LocalDef003.{u_1, u_2} Direction Cell grid)
          LocalDef007))
        ...
        (And
          (@Exists.{u_1 + 1} Direction fun (first : Direction) =>
            And
              (@Membership.mem.{u_1, u_1} Direction (List.{u_1} Direction) (@List.instMembership.{u_1} Direction)
                (@LocalDef002.{u_1, u_2} Direction Cell grid) first)
          ...
          (And
            ...
            (And
              (∀ (direction : Direction) (cellAverages : Cell → E) (cell : Cell),
                @Eq.{u_3 + 1} E
                  (@LocalDef015.{u_1, u_2, u_3} Direction Cell E inst inst_1 grid
                    ...
                    (@HSMul.hSMul.{0, u_3, u_3} Real E E
                      ...
                      (@HSub.hSub.{u_3, u_3, u_3} E E E
                        (@instHSub.{u_3} E
                          (@SubNegMonoid.toSub.{u_3} E
                            (@AddGroup.toSubNegMonoid.{u_3} E (@AddCommGroup.toAddGroup.{u_3} E inst))))
                        (@LocalDef011.{u_1, u_3} Direction E
                          (methods direction) (cellAverages cell)
                          (cellAverages
                            (@LocalDef005.{u_1, u_2} Direction Cell grid
                              (@LocalDef010.{u_1, u_3} Direction E
                                (methods direction))
                              cell)))
                        (@LocalDef011.{u_1, u_3} Direction E
                          (methods direction)
                          (cellAverages
                            (@LocalDef004.{u_1, u_2} Direction Cell grid
                              (@LocalDef010.{u_1, u_3} Direction E
                                (methods direction))
                              cell))
                          (cellAverages cell))))))
              (And
                ...
                (∀ (cellAverages : Cell → E),
                  @Eq.{max (u_2 + 1) (u_3 + 1)} (Cell → E)
                    (@LocalDef016.{u_1, u_2, u_3} Direction Cell E inst inst_1 grid methods
                      directionOrder cellAverages)
                    (@LocalDef020.{max u_3 u_2} (Cell → E)
  ...
  (have splitAdvance : (cellAverages : Prod.{0, 0} Int Int → Real) → Prod.{0, 0} Int Int → Real :=
    @LocalDef016.{0, 0, 0} Bool (Prod.{0, 0} Int Int) Real Real.instAddCommGroup
      (@Semiring.toModule.{0} Real Real.semiring) LocalDef017
      LocalDef018
    ...
    cellAverages
      (@Prod.mk.{0, 0} Int Int
        ...
        (@HSub.hSub.{0, 0, 0} Int Int Int (@instHSub.{0} Int Int.instSub) (@Prod.snd.{0, 0} Int Int cell)
          (@OfNat.ofNat.{0} Int (nat_lit 1) (@instOfNat (nat_lit 1)))));
  And
    (@LocalDef014.{0} (Prod.{0, 0} Int Int → Real) splitAdvance diagonalReference
      (@Set.univ.{0} (Prod.{0, 0} Int Int → Real)))
    (@LocalDef013.{0} (Prod.{0, 0} Int Int → Real) splitAdvance
      LocalDef019 fun (cell : Prod.{0, 0} Int Int) =>
...
```lean
{Direction : Type u_1} → {Cell : Type u_2} → LocalDef001 Direction Cell → List Direction
```
...
```lean
{Direction : Type u_1} →
  {Cell : Type u_2} → LocalDef001 Direction Cell → LocalDef006
```
...
```lean
{Direction : Type u_1} →
  {Cell : Type u_2} → LocalDef001 Direction Cell → Direction → Cell → Cell
```
...
```lean
{Direction : Type u_1} →
  {E : Type u_2} → LocalDef009 Direction E → Direction
```
...
```lean
{Direction : Type u_1} →
  {E : Type u_2} → LocalDef009 Direction E → E → E → E
```
...
```lean
{Direction : Type u_1} → {E : Type u_2} → LocalDef009 Direction E → Real
```
...
{Direction : Type u_1} →
  {Cell : Type u_2} →
    {E : Type u_3} →
      [inst : AddCommGroup E] →
        [Module Real E] →
          LocalDef001 Direction Cell →
            LocalDef009 Direction E → (Cell → E) → Cell → E
```
  ...
  {Cell : Type u_2} →
    {E : Type u_3} →
      [inst : AddCommGroup E] →
        [Module Real E] →
          LocalDef001 Direction Cell →
            (Direction → LocalDef009 Direction E) →
...
fun {Direction} {Cell} {E} [AddCommGroup E] [Module Real E] grid methods directionOrder cellAverages =>
  LocalDef020
    (List.map (fun direction => LocalDef015 grid (methods direction)) directionOrder)
    cellAverages
```
...
```lean
LocalDef001 Bool (Prod Int Int)
```

Definition body (one-level semantic boundary):

```lean
{ geometry := LocalDef008,
  coordinateDirections := List.cons Bool.false (List.cons Bool.true List.nil),
  directionsNodup := LocalDef023,
  hasTwoDirections := LocalDef024,
  leftNeighbor := fun direction cell =>
    LocalDef027 (fun direction => Prod Int Int) direction
  ...
  right_left := LocalDef025,
  left_right := LocalDef026 }
```
...
fun direction =>
  { direction := direction, timeStepOverCellWidth := 1,
    positiveTimeStep := LocalDef028, physicalFlux := id,
