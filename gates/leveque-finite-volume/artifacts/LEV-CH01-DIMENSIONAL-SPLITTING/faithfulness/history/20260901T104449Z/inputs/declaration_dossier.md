...
SHA-256: `78473a029229ead095aff423a86f10ed7bfb70e52950d9f1b88b94bfbb5a6bbe`
...
/-- A coordinate grid carries actual left/right cell adjacency in every named
direction.  The inverse laws are the combinatorial content of rectangular or
logically rectangular indexing; `geometry` records whether that indexing is
also the physical geometry or only its logical connectivity. -/
structure CoordinateFiniteVolumeGrid (Direction Cell : Type*) where
  geometry : CoordinateGridGeometry
  coordinateDirections : List Direction
  directionsNodup : coordinateDirections.Nodup
  hasTwoDirections :
    ∃ first ∈ coordinateDirections,
      ∃ second ∈ coordinateDirections, first ≠ second
  leftNeighbor : Direction → Cell → Cell
  rightNeighbor : Direction → Cell → Cell
  right_left : ∀ direction cell,
    rightNeighbor direction (leftNeighbor direction cell) = cell
  left_right : ∀ direction cell,
    leftNeighbor direction (rightNeighbor direction cell) = cell

/-- A one-dimensional finite-volume method to be used on coordinate lines.

Unlike a predicate that merely labels an arbitrary endomorphism, this record
contains a physical flux, a consistent two-state numerical flux, and the CFL
scale used by the conservative flux-difference update.  The Chapter 1 passage
treats the method's high-resolution construction as inherited from the
one-dimensional solver, so the reusable interface records precisely the
conservative semantics needed by dimensional splitting. -/
structure HighResolutionCoordinateFiniteVolumeMethod
    (Direction E : Type*) where
  direction : Direction
  timeStepOverCellWidth : ℝ
  positiveTimeStep : 0 < timeStepOverCellWidth
  physicalFlux : E → E
  numericalFlux : E → E → E
  consistent : ∀ state, numericalFlux state state = physicalFlux state

/-- Apply one conservative one-dimensional finite-volume update along a
coordinate line of a rectangular or logically rectangular grid. -/
def coordinateFiniteVolumeAdvance
    {Direction Cell E : Type*} [AddCommGroup E] [Module ℝ E]
    (grid : CoordinateFiniteVolumeGrid Direction Cell)
    (method : HighResolutionCoordinateFiniteVolumeMethod Direction E)
    (cellAverages : Cell → E) (cell : Cell) : E :=
  cellAverages cell - method.timeStepOverCellWidth •
    (method.numericalFlux
        (cellAverages cell)
        (cellAverages (grid.rightNeighbor method.direction cell)) -
      method.numericalFlux
        (cellAverages (grid.leftNeighbor method.direction cell))
        (cellAverages cell))

/-- Apply the inherited one-dimensional finite-volume methods in the listed
coordinate order. -/
def coordinateFiniteVolumeSweep
    {Direction Cell E : Type*} [AddCommGroup E] [Module ℝ E]
    (grid : CoordinateFiniteVolumeGrid Direction Cell)
    (methods : Direction →
      HighResolutionCoordinateFiniteVolumeMethod Direction E)
    (directionOrder : List Direction) (cellAverages : Cell → E) : Cell → E :=
  orderedOperatorSweep
    (directionOrder.map fun direction =>
      coordinateFiniteVolumeAdvance grid (methods direction))
    cellAverages

/-- Splitting is effective for a reference evolution on a problem family when
the split and reference advances agree on every member of that family. -/
def SplitMatchesReferenceOn
    {State : Type*} (splitAdvance referenceAdvance : State → State)
    (problemFamily : Set State) : Prop :=
  ∀ state ∈ problemFamily, splitAdvance state = referenceAdvance state

/-- A concrete state requires information beyond a given split advance when
the split update fails to reproduce the specified coupled reference update. -/
def RequiresFullyMultidimensionalAt
    {State : Type*} (splitAdvance referenceAdvance : State → State)
    (state : State) : Prop :=
  splitAdvance state ≠ referenceAdvance state

/-- Consistency makes every coordinate update preserve a constant state. -/
theorem coordinateFiniteVolumeAdvance_const
    {Direction Cell E : Type*} [AddCommGroup E] [Module ℝ E]
    (grid : CoordinateFiniteVolumeGrid Direction Cell)
    (method : HighResolutionCoordinateFiniteVolumeMethod Direction E)
    (state : E) :
    coordinateFiniteVolumeAdvance grid method (fun _ => state) =
      (fun _ => state) := by
  funext cell
  simp [coordinateFiniteVolumeAdvance, method.consistent]
